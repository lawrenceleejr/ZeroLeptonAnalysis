...
ftree = TFile("../DataFiles/FT/data.root")
ftree.cd()
intree = {}

intree["SR"] = ftree.Get("trees_SR_")
...
for regionName in intree:
	...
	skimtree["SRG3b"] = preselectiontree.CopyTree("(MET>150)*( deltaQCD > 0)* ( RPT_HT5PP < 0.08  )* ( R_H2PP_H5PP > 0.2)* ( R_HT5PP_H5PP > 0.65)* ( RPZ_HT5PP < 0.6)* ( minR_pTj2i_HT3PPi > 0.08)* ( maxR_H1PPi_H2PPi < 0.98)* ( H2PP > 900)*(HT5PP>2800)").Clone("SRG3b")
	...
	for itree,mytree in enumerate(skimtree):
		skimtree[mytree].Draw(var+">>h%d(20,0,50)"%itree)
		...
		h_eff[mytree].SetMinimum(1e-8)
		h_eff[mytree].SetMaximum(1)
