   ...
   int discexcl = 1)
{
  ...
  TH2F* histecls0zz= (TH2F*)f0->Get("sigp1clsf");
  ...
  TGraph* gr_contour_1su = ContourGraph( hist0_1su )->Clone();
  TGraph* gr_contour_1sd = ContourGraph( hist0_1sd )->Clone();
  ...
  DrawContourLine95( leg, contour_1su,         "", c_myYellow, 1, 2 );
  DrawContourLine95( leg, contour_1sd,         "", c_myYellow, 1, 2 );
  ...
  if (contour_obscls_0!=0) DrawContourLine95( leg, contour_obscls_0, "Observed limit", CombinationGlob::c_DarkRed, 1, 4); 

  DrawContourLine95( leg, contourobs_comb, "", c_myExp, 6, 2);
  ...
  // mass forbidden line
  TLatex massforbidden = TLatex(300,380, STmass+" < m_{#tilde{#chi}^{0}_{1}}");
  ...
  Leg2->DrawLatex(0.13, 0.6, "All limits at 95% CL.");
  Leg2->AppendPad();
  ...
  Leg3->DrawLatex(0.13, 0.65, "(Fit to SR,CRT,CRW,CRY)");
  Leg3->AppendPad();
...
void Show_SR(TString oredList,  TCanvas *can, float xlow, float xhigh, float ylow, float yhigh, TString Grid)
{
    ...
    for( Int_t i = 0; i < tree->GetEntries(); i++ ){
        ...
        cout <<"BestSR for "<< m0 << " " << m12 << " is: " << fID <<" "<<mySR<< endl;
