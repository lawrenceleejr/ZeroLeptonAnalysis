...
void SUSY_nGM_m0_vs_m12_all_withBand_cls( TString fname0="", TString fname1="",
        TString fname2="",
        TString gridname="",
        ...
        TString fnameMass= "contourmacros/mSugraGridtanbeta10_gluinoSquarkMasses.root")
{
    ...
    TH2F *frame = new TH2F("frame", "m_{gluino} vs. m_{lsp} - ATLAS work in progress", 100, 115.,plotrange, 100, 390., 1300. );
    // set common frame style
    CombinationGlob::SetFrameStyle2D( frame, 1.0 ); // the size (scale) is 1.0
    
    frame->SetXTitle( "m_{#tilde{#tau}} [GeV]" );
    frame->SetYTitle( "m_{#tilde{g}} [GeV]" );
    ...
    TString plottitle = "Natural Gauge Mediated Model, #tilde{#tau} is NLSP,m_{#tilde{q}}>>m_{#tilde{g}},  #mu=400 GeV";
