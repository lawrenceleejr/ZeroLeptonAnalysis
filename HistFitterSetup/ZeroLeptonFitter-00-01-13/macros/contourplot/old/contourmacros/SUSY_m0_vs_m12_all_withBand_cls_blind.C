...
*/
  ...
  if (discexcl==1) {
     ...
     if (!extExpectation) { // expectation from toys
        ...
        if (showfixSigXSecBand) {
           // blind if (contour_esigxsp1s!=0) DrawContourLine95( leg, contour_esigxsp1s, "", CombinationGlob::c_DarkRed, 3, 2 );
        }
        // blind if (contour_obscls!=0) DrawContourLine95( leg, contour_obscls, "Observed limit (#pm1 #sigma^{SUSY}_{theory})", CombinationGlob::c_DarkRed, 1, 4);
        if (showfixSigXSecBand) {
           // blind if (contour_esigxsm1s!=0) DrawContourLine95( leg, contour_esigxsm1s, "", CombinationGlob::c_DarkRed, 3, 2 );
        }
  ...
  if (showtevatron==1 && discexcl==1) {
     ...
     //leg->AddEntry( noGRE, "LEP2 #tilde{#chi}^{#pm}_{1}","F");
