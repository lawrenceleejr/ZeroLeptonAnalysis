...
#INPUTDIR="%s/../ZL_00-01-08/optimisation/optimisation-GG_onestepCC-20151115-192904/results/" % (os.getenv("ZEROLEPTONFITTER")) 
INPUTDIR="%s/../ZL_00-01-08/optimisation/optimisation-GG_direct-20151115-192557/results/" % (os.getenv("ZEROLEPTONFITTER"))
#INPUTDIR="%s/../ZL_00-01-08/optimisation/optimisation-SS_direct-20151115-192616/results/" % (os.getenv("ZEROLEPTONFITTER"))
...
def parseCmdLine(args):
    ...
    parser.add_option("--grid", dest="grid", help="grid name SS_direct, GG_direct or GG_onestepCC", default="GG_direct") # "GG_onestepCC") #"SS_direct" #
...
def MergeFiles(config):
    ...
    if grid_name == "SM_GG_onestep_LSP60":
    ...
    elif grid_name == "GG_direct" or grid_name == "SS_direct" or grid_name == "GG_onestep_X05" or grid_name =="GG_onestepCC" or grid_name =="GG_onestepCC_LSP60":
        config.anaList = [
        ...
        config.anaList = ["SR2jl", "SR2jc", "SR2jt", "SR4jt", "SR5j", "SR6jm", "SR6jt"]
        #config.anaList = ["SR2jl", "SR2jm", "SR2jt", "SR4jt", "SR5j", "SR6jm", "SR6jt"]
        
        if grid_name == "GG_onestep_X05" : grid_name="GG_onestepCC"
    else:
    ...
    for ana in config.anaList:        
        ...
        for xs in allXS:
            ...
            if config.grid.find("GG_onestepCC")!=-1:
                ...
                interpretation = "mgluino:mchargino:mlsp"; 
                #interpretation = "mgluino:mlsp:mchargino"; 
            if config.discovery:
    ...
    for aLine in allLines:
        ...
        if config.discovery:
            ...
            for sigma, pzerovalue in enumerate(values):
                ...
                tlines[sigma].SetLineColor(TColor.kGray+2)
            ...
            for sigma, ysigma in enumerate([0.725, 0.65, 0.545, 0.4, 0.22]):
                ...
                lsigma[sigma].SetTextColor(TColor.kGray+2);
...
def CompareLines(config, fileList, outputName):
    ...
    for aLine in allLines:
        ...
        colors = [0,1,ROOT.kRed+2,TColor.kGreen+3,TColor.kBlue+1,6,2,7,15,50,35,42,27,38,ROOT.kPink,45,56]*100
        counter=0
        ...
        if config.discovery:
            ...
            for sigma, pzerovalue in enumerate(values):
                ...
                tlines[sigma].SetLineStyle(7)
                tlines[sigma].SetLineWidth(2)
                tlines[sigma].SetLineColor(TColor.kGray+1)
                tlines[sigma].Draw("same")

            lsigma=[]
            for sigma, ysigma in enumerate([0.725, 0.65, 0.545, 0.4, 0.22]):
                lsigma.append(TLatex(0.91,ysigma,str((int(sigma)+1))+" #sigma"))
                lsigma[sigma].SetTextColor(TColor.kGray+1);
                lsigma[sigma].SetNDC(kTRUE);
                lsigma[sigma].SetTextSize(0.04);
                lsigma[sigma].SetTextFont(42);
                lsigma[sigma].Draw("same");
...
def MakePlots(config):
    ...
    #for ana in ["combined_cutcount", "combined_5bin"]: #combine multiple by hand
        ...
        DrawContourLine3sigma( leg_each, contour, '3#sigma evidence', TColor.kRed+2,   1, 2);
        DrawContourLine2sigma( leg_each, contour, '2#sigma', TColor.kBlue+1,  7, 2);
        DrawContourLine1sigma( leg_each, contour, '1#sigma', TColor.kGreen+3, 3, 2);
    ...
    for xsecStr in allXS:
        ...
        for indx,ana in enumerate(config.anaList):
            ...
            for line in f.readlines():
                ...
                key = "%d_%d" % (par1, par2)
                par4 = int( vals[allpar.index("mode")]) 
