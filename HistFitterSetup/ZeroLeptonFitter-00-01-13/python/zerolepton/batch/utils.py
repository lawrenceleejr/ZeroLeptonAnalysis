...
def runBatchCommands(args, outputDir, commands, runMissing=False, forceOverwrite=False):
    ...
    for i, x in enumerate(commands):
        if runMissing:
            filename = '%s/Jobs/%s-MISSING-%d.sh' % (os.getenv('ZEROLEPTONFITTER'), os.path.basename(outputDir), i+1)
        else:
    ...
    if useMultiple:
        for i, c in enumerate(chunks(jobFilenames, batchConfig['number_to_combine'])):
            if runMissing:
                filename = '%s/Jobs/%s-combined-MISSING-%d.sh' % (os.getenv('ZEROLEPTONFITTER'), os.path.basename(outputDir), i+1)
            else:
