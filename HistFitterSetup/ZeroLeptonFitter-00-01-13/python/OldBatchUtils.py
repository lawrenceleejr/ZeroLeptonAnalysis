...
#
###########################################################################
myscript= "analysis/ZeroLepton_Run2.py"
...
#    f.write("if [ ! -d data ]; then mkdir -v data; fi\n")
    ...
    if config.mult==False:
        ...
        #f.write("/bin/cp -rf results/*hypo*root "+MYWORKINGDIR+"results/\n")
        #f.write("/bin/cp -rf results/*upperlimit*root "+MYWORKINGDIR+"results/\n")
        if config.saveWS:
        ...
        for point in points:
            ...
            cmd="HistFitter.py "+config.option+" -g "+option2+" -r "+ana+" "+MYWORKINGDIR+myscript+"  "
            if config.doSubmission==False:
                print cmd+" >& "+ana+option2+".log"
            #f.write("echo "+cmd+" \n")
            f.write(cmd+" \n")
