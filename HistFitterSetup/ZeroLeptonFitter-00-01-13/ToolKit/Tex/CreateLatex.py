...
oldkeys=finalChannelsDict.keys()
keys=[]
for key in oldkeys:
    keys.append(key.replace("-0.","-0"))


for ana in sorted(keys):
    fname = "yield_{0}".format(ana)
    if not os.path.exists(fname+".tex"): continue
    f.write("\\include{%s}\n" % fname)

f.write("\\clearpage\n")
for ana in sorted(keys):
    f.write("\\clearpage\n")
    fname = "systtable_{0}_SR_cuts".format(ana)
    if not os.path.exists(fname+".tex"): continue
    f.write("\\include{%s}\n" % fname)

# f.write("\\clearpage\n")
# for ana in sorted(keys):
#     fname = "systtable_{0}_VRWMf".format(ana)
#     print fname
#     if not os.path.exists(fname+".tex"): continue
#     f.write("\\include{%s}\n" % fname)
...
for ana in sorted(keys):
    f.write("\\clearpage\n")
    fname = "histpull_{0}".format(ana)+".pdf"
    if not os.path.exists(fname): continue
    print fname
    f.write("\\begin{figure}[h]\\begin{center}\\begin{tabular}{cc}\\includegraphics[width=1\\textwidth]{%s}\\end{tabular}\\end{center}\\end{figure}\n" % fname)
    
f.write("\\clearpage\n")
for ana in sorted(keys):
    f.write("\\clearpage\n")
    fname = "MU_{0}".format(ana)+".pdf"
    if not os.path.exists(fname): continue
    f.write("\\begin{figure}[h]\\begin{center}\\begin{tabular}{cc}\\includegraphics[width=1\\textwidth]{%s}\\end{tabular}\\end{center}\\end{figure}\n" % fname)
