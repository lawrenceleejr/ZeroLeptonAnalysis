...
////#include "contourmacros/0lep_Moriond_mglmsqexcl.C"
#include "0lep_Moriond_mglmsqexcl-4.C"

void SUSY_mgl_vs_msq_all_withBand_cls_blind_forpMSSM( TString fname0 = "mudat_list.root",// nominal
                                      TString fname1 = "",               // Up
                                      TString fname2 = "",               // Down  
                                      ...
                                      TString fnameMass= "../../../HistFitterUser/common/mSugraGridtanbeta10_gluinoSquarkMasses.root")
{
  ...
  // create and draw the frame
   TH2F *frame = new TH2F("frame", "m_{0} vs m_{12} - ATLAS for approval", 100, 800., 2400. ,100, 800., 2800.);
   //TH2F *frame = new TH2F("frame", "m_{0} vs m_{12} - ATLAS work in progress", 100, 1000., 2400. ,100, 1000., 2400.);

   bool plotMoriond=true;
   //if (plotMoriond ) frame = new TH2F("frame", "m_{0} vs m_{12} - ATLAS for approval", 100, 800., 2000. ,100, 800., 2000.);
  ...
  TLegend *leg = new TLegend (0.59,0.46,0.84,0.66);
  //TLegend *leg = new TLegend (0.64,0.8,0.92,0.915); 
...
*/
  ...
  */
     ...
     for (int xbin = 1; xbin <= contour_obscls->GetNbinsX(); xbin++) {
        for (int ybin = 1; ybin <= contour_obscls->GetNbinsY(); ybin++) {
         if ( ax->GetBinCenter(xbin) > 700. && ax->GetBinCenter(xbin) < 940.
            ...
            //if (contour_obscls->GetBinContent(xbin,ybin)<1.65) contour_obscls->SetBinContent(xbin, ybin, 2.0);
  ...
  if (plotMoriond ){
     ...
     //Moriond_exp = zl_Moriond_mglmsqexcl_exp();
  }
  ...
  if (discexcl==1) {
     ...
     if (!extExpectation) { // expectation from toys
        ...
        TLine* line1  = new TLine( 1730, 1982, 1811, 1982);
        line1->SetLineWidth(2);
        line1->SetLineColor(CombinationGlob::c_DarkRed);
        line1->SetLineStyle(3);
        line1->Draw("SAME") ;
        TLine* line2  = new TLine( 1730, 2088, 1811, 2088);
        line2->SetLineWidth(2);
        line2->SetLineColor(CombinationGlob::c_DarkRed);
        line2->SetLineStyle(3);
        line2->Draw("SAME") ;                
        
        if (contour_expcls!=0) DrawContourLine95( leg, contour_expcls, "", c_myExp, 6 ); 
  ...
  TLatex *Leg0 = new TLatex( xmin, ymax + dy*0.025, "Squark-gluino-neutralino model, m(#tilde{#chi}_{1}^{0}) = 0 GeV" );
  ...
  Leg1->SetTextColor( 1 );
  if (plotMoriond ){
     Leg1->DrawLatex(0.60,0.75, Form("#int L dt = %1.1f fb^{-1},  #sqrt{s}=8 TeV",lumi));  // 0.32,0.87
  }else{
     Leg1->DrawLatex(0.60,0.75, Form("#int L dt = %1.1f fb^{-1},  #sqrt{s}=8 TeV",lumi));  // 0.32,0.87
  }
  Leg1->AppendPad();
