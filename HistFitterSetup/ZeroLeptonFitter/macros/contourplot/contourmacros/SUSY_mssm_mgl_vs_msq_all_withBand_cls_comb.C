...
void SUSY_mssm_mgl_vs_msq_all_withBand_cls_comb( TString fname0 = "Outputs.lsp0/mssm_combined_fixSigXSecNominal__1_harvest_list.root",// nominal
                                      TString fname1 = "Outputs.lsp0/mssm_combined_fixSigXSecUp__1_harvest_list.root",               // Up
                                      TString fname2 = "Outputs.lsp0/mssm_combined_fixSigXSecDown__1_harvest_list.root",               // Down  
                                      TString fname3 = "", // external expection
                                      TString fname0_lsp2 = "Outputs.lsp395/mssm_combined_fixSigXSecNominal__1_harvest_list.root",               // lsp2
                                      TString fname0_lsp3 = "Outputs.lsp695/mssm_combined_fixSigXSecNominal__1_harvest_list.root",               // lsp3
                                      const char* prefix="",
                                      const float lumi = 20.3,
				      const double lspmass = 0,
				      const double lspmass_lsp2 = 395,
				      const double lspmass_lsp3 = 695,
                                      bool showsig = true,
                                      int discexcl = 1,
				      ...
				      TString hname6 = "sigp1ref")
{
   ...
   cout << "--- Reading root base file: " << fname0_lsp2 << endl;
   TFile* f0_lsp2 = TFile::Open( fname0_lsp2, "READ" );
   if (!f0_lsp2) {
      cout << "*** Error: could not retrieve histogram: " << hname0 << " in file: " << f0_lsp2->GetName() 
           << " ==> abort macro execution" << endl;
      return;
   }
   cout << "--- Reading root base file: " << fname0_lsp3 << endl;
   TFile* f0_lsp3 = TFile::Open( fname0_lsp3, "READ" );
   if (!f0_lsp3) {
      cout << "*** Error: could not retrieve histogram: " << hname0 << " in file: " << f0_lsp3->GetName() 
           << " ==> abort macro execution" << endl;
      return;
   }
   ...
   TH2F* histecls_lsp2 = (TH2F*)f0_lsp2->Get( "sigp1expclsf" ); 
   TH2F* histocls_lsp2 = (TH2F*)f0_lsp2->Get( "sigp1clsf" ); 
   if (histecls_lsp2!=0) histecls_lsp2->SetDirectory(0);
   if (histocls_lsp2!=0) histocls_lsp2->SetDirectory(0);

   TH2F* histecls_lsp3 = (TH2F*)f0_lsp3->Get( "sigp1expclsf" ); 
   TH2F* histocls_lsp3 = (TH2F*)f0_lsp3->Get( "sigp1clsf" ); 
   if (histecls_lsp3!=0) histecls_lsp3->SetDirectory(0);
   if (histocls_lsp3!=0) histocls_lsp3->SetDirectory(0);
   ...
   TH2F* hist0_lsp2 = (TH2F*)f0_lsp2->Get( hname0 );
   TH2F* hist1_lsp2 = (TH2F*)f0_lsp2->Get( hname1 );
   TH2F* hist3_lsp2 = (TH2F*)f0_lsp2->Get( hname3 );
   TH2F* hist5_lsp2 = (TH2F*)f0_lsp2->Get( hname5 );
   TH2F* hist6_lsp2 = (TH2F*)f0_lsp2->Get( hname6 );
   
   if (hist0_lsp2!=0) hist0_lsp2->SetDirectory(0);
   if (hist1_lsp2!=0) hist1_lsp2->SetDirectory(0);
   if (hist3_lsp2!=0) hist3_lsp2->SetDirectory(0);
   if (hist5_lsp2!=0) hist5_lsp2->SetDirectory(0);
   if (hist6_lsp2!=0) hist6_lsp2->SetDirectory(0);
   f0_lsp2->Close();

   TH2F* hist0_lsp3 = (TH2F*)f0_lsp3->Get( hname0 );
   TH2F* hist1_lsp3 = (TH2F*)f0_lsp3->Get( hname1 );
   TH2F* hist3_lsp3 = (TH2F*)f0_lsp3->Get( hname3 );
   TH2F* hist5_lsp3 = (TH2F*)f0_lsp3->Get( hname5 );
   TH2F* hist6_lsp3 = (TH2F*)f0_lsp3->Get( hname6 );
   
   if (hist0_lsp3!=0) hist0_lsp3->SetDirectory(0);
   if (hist1_lsp3!=0) hist1_lsp3->SetDirectory(0);
   if (hist3_lsp3!=0) hist3_lsp3->SetDirectory(0);
   if (hist5_lsp3!=0) hist5_lsp3->SetDirectory(0);
   if (hist6_lsp3!=0) hist6_lsp3->SetDirectory(0);
   f0_lsp3->Close();
   ...
   TH2F* contour_lsp2         = ( hist1_lsp2!=0 ? FixAndSetBorders( *hist1_lsp2, "contour", "contour", 0 ) : 0);
   TH2F* contour_obs_lsp2     = ( hist0_lsp2!=0 ? FixAndSetBorders( *hist0_lsp2, "contour_obs", "contour_obs") : 0 );
   
   TH2F* contour_ep1s_lsp2    = ( hist3_lsp2!=0 ? FixAndSetBorders( *hist3_lsp2, "contour", "contour", 0 ) : 0 );
   TH2F* contour_em1s_lsp2    = ( hist5_lsp2!=0 ? FixAndSetBorders( *hist5_lsp2, "contour", "contour", 0 ) : 0 );


   TH2F* contour_lsp3         = ( hist1_lsp3!=0 ? FixAndSetBorders( *hist1_lsp3, "contour", "contour", 0 ) : 0);
   TH2F* contour_obs_lsp3     = ( hist0_lsp3!=0 ? FixAndSetBorders( *hist0_lsp3, "contour_obs", "contour_obs") : 0 );
   
   TH2F* contour_ep1s_lsp3    = ( hist3_lsp3!=0 ? FixAndSetBorders( *hist3_lsp3, "contour", "contour", 0 ) : 0 );
   TH2F* contour_em1s_lsp3    = ( hist5_lsp3!=0 ? FixAndSetBorders( *hist5_lsp3, "contour", "contour", 0 ) : 0 );
   ...
   TH2F* contour_expcls_lsp2(0);
   if (histecls_lsp2!=0)     { contour_expcls_lsp2     = FixAndSetBorders( *histecls_lsp2, "contour_expcls", "contour_expcls", 0 ); }
   TH2F* contour_obscls_lsp2(0);
   if (histocls_lsp2!=0)     { contour_obscls_lsp2     = FixAndSetBorders( *histocls_lsp2, "contour_obscls", "contour_obscls", 0 ); }

   TH2F* contour_expcls_lsp3(0);
   if (histecls_lsp3!=0)     { contour_expcls_lsp3     = FixAndSetBorders( *histecls_lsp3, "contour_expcls", "contour_expcls", 0 ); }
   TH2F* contour_obscls_lsp3(0);
   if (histocls_lsp3!=0)     { contour_obscls_lsp3     = FixAndSetBorders( *histocls_lsp3, "contour_obscls", "contour_obscls", 0 ); }
  ...
  TGraph* gr_contour_ep1s_lsp2 = ContourGraph( contour_ep1s_lsp2 )->Clone();
  TGraph* gr_contour_em1s_lsp2 = ContourGraph( contour_em1s_lsp2 )->Clone();

  TGraph* gr_contour_ep1s_lsp3 = ContourGraph( contour_ep1s_lsp3 )->Clone();
  TGraph* gr_contour_em1s_lsp3 = ContourGraph( contour_em1s_lsp3 )->Clone();
   ...
   if (hist1_lsp2!=0) hist1_lsp2->SetMarkerStyle(21);
   if (hist1_lsp2!=0) hist1_lsp2->SetMarkerSize(1.5);
   if (hist1_lsp3!=0) hist1_lsp3->SetMarkerStyle(21);
   if (hist1_lsp3!=0) hist1_lsp3->SetMarkerSize(1.5);
   Float_t nsigmax(0)
  ...
  // create and draw the frame
   TH2F *frame = new TH2F("frame", "m_{gl} vs m_{sq} - ATLAS for approval", 100, 800., 2400. ,100, 800., 2800.);
  ...
  Int_t nsigma=2;
  TLegend *leg = new TLegend (0.50,0.58,0.85,0.91);
  ...
  TGraph* grshadeExp = DrawExpectedBand_new( gr_contour_ep1s, gr_contour_em1s, CombinationGlob::c_DarkYellow , 1001   , 800, 3000, 800, 3000, 0)->Clone();
  ...
  TGraph* zl_obs=0;
  if(plotMoriond){
    zl_obs = zl_Moriond_mglmsqexcl_obs();
    zl_obs->SetFillStyle(4001);
    zl_obs->SetFillColor(kBlue-10);
    zl_obs->SetLineWidth(2);
    zl_obs->SetLineColor(kGray+1);
    zl_obs->Draw("FL");
    TGraph* zl_obs_frame = (TGraph*)zl_obs->Clone("zl_obs");
    zl_obs_frame->Draw("L");
  }
  ...
  if (discexcl==1) {
	 ...
	 DrawContourLine95( leg, contour_expcls_lsp2, "", kAzure+1, 6 );
	 DrawContourLine95( leg, contour_expcls_lsp3, "", kGreen+3, 6 );
       }
        ...
        if (showOneSigmaExpBand) {
           ...
           DummyLegendExpected(leg, Form("m(#tilde{#chi}^{0}_{1}) = %.0f GeV Expected limit (#pm1 #sigma_{exp})",lspmass), c_myYellow, 1001, CombinationGlob::c_DarkBlueT3, 6, 2);
	...
	if(!blind){
	  if (showfixSigXSecBand) {
	    ...
	    if (contour_esigxsm1s!=0) DrawContourLine95( leg, contour_esigxsm1s, "", c_myRed, 3, 2 );
          ...
          if (contour_obscls!=0) DrawContourLine95( leg, contour_obscls, Form("m(#tilde{#chi}^{0}_{1}) = %.0f GeV Observed limit (#pm1 #sigma^{SUSY}_{theory})",lspmass), c_myRed, 1, 4);
	}

        if (showOneSigmaExpBand) {
	   DummyLegendExpected(leg, Form("m(#tilde{#chi}^{0}_{1}) = %.0f GeV Expected limit",lspmass_lsp2), 0, 0, kAzure+1, 6, 2);
        }
	if(!blind){
	  if (contour_obscls_lsp2!=0) DrawContourLine95( leg, contour_obscls_lsp2, Form("m(#tilde{#chi}^{0}_{1}) = %.0f GeV Observed limit",lspmass_lsp2), kAzure+1, 1, 4);
	}
        if (showOneSigmaExpBand) {
	   DummyLegendExpected(leg, Form("m(#tilde{#chi}^{0}_{1}) = %.0f GeV Expected limit",lspmass_lsp3), 0, 0, kGreen+3, 6, 2);
        }
	if(!blind){
	  if (contour_obscls_lsp3!=0) DrawContourLine95( leg, contour_obscls_lsp3, Form("m(#tilde{#chi}^{0}_{1}) = %.0f GeV Observed limit",lspmass_lsp3), kGreen+3, 1, 4);
	}
        if(plotMoriond) leg->AddEntry(zl_obs,"7TeV (4.7fb^{-1}) m(#tilde{#chi}^{0}_{1}) = 0 GeV Observed","f");
  ...
  TLatex *Leg0 = new TLatex( xmin, ymax + dy*0.025, Form("Squark-gluino-neutralino model") );
  ...
  Leg1->SetTextColor( 1 );
  Leg1->DrawLatex(0.14,0.79, Form("#int L dt = %1.1f fb^{-1},  #sqrt{s}=8 TeV",lumi));  // 0.32,0.87
  Leg1->AppendPad();
  ...
  Leg2->SetTextFont(42);
  if (prefix!=0) {
    Leg2->DrawLatex(0.6,0.68,prefix); // 0.15,0.81
    Leg2->AppendPad(); 
  }
  ...
  leg3->DrawLatex(0.15, 0.72, Form("0-lepton combined"));   // 0.27,0.87
  leg3->AppendPad();
  ...
  //prel->DrawLatex(0.27, 0.87, "for approval");   // 0.27,0.87
  ...
  CombinationGlob::imgconv( c, Form("plots/mglmsq_comb_%s",outfile.Data()) );  
}
...
ContourGraph( TH2F* hist)
{
   ...
   h->GetYaxis()->SetRangeUser(800,3000);
   h->GetXaxis()->SetRangeUser(800,3000);
...
void Show_SR(TString oredList,  TCanvas *can, float xlow, float xhigh, float ylow, float yhigh, bool useShape)
{
    ...
    tree->SetBranchAddress("mgl", &mgl, &b_mgl);
    ...
    for( Int_t i = 0; i < tree->GetEntries(); i++ ){
        ...
        // remove legend region
        if( mgl > 1700 && msq > 1900 ) continue;

        // be 10% outside the edges
        if( (mgl > (xhigh-xlow)/30.0 + xlow) &&
                (mgl < xhigh - (xhigh-xlow)/30.0) &&
                (msq > (yhigh-ylow)/30.0 + ylow) &&
                (msq < yhigh - (yhigh-ylow)/30.0)) {
            lat.DrawLatex(mgl, msq, mySR.Data());
        }
