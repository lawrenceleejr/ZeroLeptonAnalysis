...
const char* SM_GG_onestep_mgluino_vs_x_nofloat(const char* textfile = 0, TH2D* inputHist =0, const char* rootfile = "mgluinox_nofloat.root", TString id1="mgluino",TString id2="x", int   nbinsX=100,int nbinsY=100, float minX=0,float maxX=1600, float minY=0,float maxY=1.5)
{
   ...
   TString correlation="(mchargino-mlsp)/(mgluino-mlsp):mgluino";
   TString fileName= textfile;
   if (fileName.Contains("SS_onestep")) correlation="(mchargino-mlsp)/(msquark-mlsp):msquark"; 
   ...
   if (inputHist!=NULL){
     ...
     //TH2D* p1clsf = DrawUtil::triwsmooth( tree, "CLs:mchargino:mgluino", "p1clsf" , "Observed CLs", "p1>0 && p1<=1", clonep1clsf );}
     TH2D* p1clsf = DrawUtil::triwsmooth( tree, "CLs:"+correlation, "p1clsf" , "Observed CLs", "p1>0 && p1<=1", clonep1clsf );}
   else{
     //TH2D* p1clsf = DrawUtil::triwsmooth( tree, "CLs:mchargino:mgluino", "p1clsf" , "Observed CLs", "p1>0 && p1<=1", inputHist );
