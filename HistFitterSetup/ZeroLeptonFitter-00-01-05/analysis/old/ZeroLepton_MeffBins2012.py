...
# Gtt grid
grid="Gtt"
allpoints = ["1400_2500_1045"] # e.g. Gtt_1400_2500_1045: m_gluino m_stop m_neutralino, m_stop=2500 fixed

##gluino-stop grid 
#grid="Gluino_Stop_charm"
#allpoints = ["700_200_1"] #e.g. Gluino_Stop_charm_900_500_1_SRAll: m_gluino m_stop m_neutralino, m_neutralino=1 fixed

#use shape factors for Top and W bkgs?
useShape = False

#number of bins in a shape fit
nBins = 5

#use the overflow bin?
useOverflowBin = True

#use validation regions? not possible in excl fit!
doValidation = False

#final cuts dictionnary; entries are (met/meff, low range, high range)
metomeffDefault=(0.3,0.25,0.25,0.20,0.15)

#original selections: cut&count cut+500Gev for range. Don't forget to use the overflow bin!
selections={
    'loose' : [(0.4,1000000,1500000),           None, (0.30,1000000,1500000),           None, (0.30,1000000,1500000)],
    'medium': [(0.4,1300000,1800000), (0.30,1300000,1800000), (0.30,1300000,1700000),           None, (0.25,1300000,1800000)],
    'tight' : [(0.3,1900000,2400000), (0.25,1900000,2400000), (0.25,1900000,2400000), (0.15,1700000,2200000), (0.15,1400000,1900000)]
    }
...
if pickedSRs != None and len(pickedSRs) >= 1: 
    ...
    if len(pickedSRs)>=2 and pickedSRs[1] in selections.keys():
        level=pickedSRs[1]
        meffLow=selections[pickedSRs[1]][chn][1]
        meffHigh=selections[pickedSRs[1]][chn][2]
...
if meffLow == None or chn > 5 or chn < 0:
...
INPUTDIR="root://eosatlas//eos/atlas/user/m/makovec/SuSy/HistFitterNtuples/ZLLCW01Jul12_v4/"
INPUTDIR="/mnt/users1/staf/gbesjes/work/HistFitterNtuples/ZLLCW01Jul12_v4/"
...
#NB do not replace with a dummy or remove - is used in normalization calculation
meffcut="(meffInc>"+str(meffLow)+" && meffInc < "+str(meffHigh)+")"
if useOverflowBin:
    meffcut="(meffInc>"+str(meffLow)+")"
...
# Signal regions
configMgr.cutsDict["SR"]    = baselineSR[chn]+" && "+dphicut[chn]+" && "+metomeffcut[chn]+" && "+meffcut+"&& (nBJet>0)"
...
for point in allpoints:
    ...
    CRGAMMA = myFitConfig.addChannel("meffInc",["CR1a"], nBins, meffLow, meffHigh)
    CRGAMMA.useOverflowBin = useOverflowBin
    ...
    #Channel
    SR = myFitConfig.addChannel("meffInc",["SR"], nBins, meffLow, meffHigh)
    SR.useOverflowBin = useOverflowBin
    if useQCD==True:
    ...
    #-------------------------------------------------
    if useQCD==True:
        CRQCD = myFitConfig.addChannel("meffInc",["CRQCD"], nBins, meffLow, meffHigh)    
        CRQCD.useOverflowBin=useOverflowBin
    ...
    CRT = myFitConfig.addChannel("meffInc",["CRT"], nBins, meffLow, meffHigh) 
    CRT.useOverflowBin=useOverflowBin
    ...
    CRW = myFitConfig.addChannel("meffInc",["CRW"], nBins, meffLow, meffHigh)
    CRW.useOverflowBin = useOverflowBin
    ...
    if doValidation:
        if not (len(pickedSRs)>=2 and pickedSRs[1].find("tight")>=0 and pickedSRs[0]=="SRD"): #no stat in SRD,tight
            VRZ = myFitConfig.addChannel("meffInc",["VRZ"], nBins, meffLow, meffHigh)
            VRZ.useOverflowBin = useOverflowBin
        ...
        if len(pickedSRs)>=2 and pickedSRs[1].find("tight")<0:
            VRZ_1b = myFitConfig.addChannel("meffInc",["VRT2L"], nBins, meffLow, meffHigh)
            VRZ_1b.useOverflowBin = useOverflowBin
...
##         myFitConfig.setValidationChannels(VRZ_1c)
        ...
        VRWT_P = myFitConfig.addChannel("meffInc",["VRWT_P"], nBins, meffLow, meffHigh)
        VRWT_P.useOverflowBin = useOverflowBin
        ...
        VRWT_M = myFitConfig.addChannel("meffInc",["VRWT_M"], nBins, meffLow, meffHigh)
        VRWT_M.useOverflowBin = useOverflowBin
        ...
        VRW1 = myFitConfig.addChannel("meffInc",["VRW1"], nBins, meffLow, meffHigh)
        VRW1.useOverflowBin = useOverflowBin
        ...
        VRT1 = myFitConfig.addChannel("meffInc",["VRT1"], nBins, meffLow, meffHigh)
        VRT1.useOverflowBin = useOverflowBin
        ...
        VRW2 = myFitConfig.addChannel("meffInc",["VRW2"], nBins, meffLow, meffHigh)
        VRW2.useOverflowBin = useOverflowBin
        ...
        VRT2 = myFitConfig.addChannel("meffInc",["VRT2"], nBins, meffLow, meffHigh)
        VRT2.useOverflowBin = useOverflowBin
        ...
        VRW3 = myFitConfig.addChannel("meffInc",["VRW3"], nBins, meffLow, meffHigh)
        VRW3.useOverflowBin = useOverflowBin
        ...
        VRT3 = myFitConfig.addChannel("meffInc",["VRT3"], nBins, meffLow, meffHigh)
        VRT3.useOverflowBin = useOverflowBin
        ...
        if useQCD==True:
            VRQCD2 = myFitConfig.addChannel("meffInc",["VRQ2"], nBins, meffLow, meffHigh)
            VRQCD2.useOverflowBin = useOverflowBin
            ...
            VRQCD1 = myFitConfig.addChannel("meffInc",["VRQ1"], nBins, meffLow, meffHigh)
            VRQCD1.useOverflowBin = useOverflowBin
