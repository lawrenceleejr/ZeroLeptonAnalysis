...
if pickedSRs!=None:  #pickedSRs is set by the "-r" HistFitter option
    ...
    if meff == None or chn > 6 or chn < 0 :
        print "No such signal region/meff cut combination!"
        print chn,meff
        print sigSamples
        sys.exit()
...
INPUTDIR="/users/staf/gbesjes/work/HistFitterNtuples/v1.1/"
...
nBinsMeff = 3 #number of bins to take in meff
bjetveto="(nBJet==0)"
bjetcut ="(nBJet>0)"

configMgr.cutsDict["SR"]    = baselineSR[chn]+" && "+dphicut[chn]+" && "+metomeffcut[chn]
configMgr.cutsDict["CRZ"]   = baselineSR[chn]
configMgr.cutsDict["CRW"]   = baselineSR[chn]+" && "+bjetveto
configMgr.cutsDict["CRTop"] = baselineSR[chn]+" && "+bjetcut
configMgr.cutsDict["CRQCD"] = baselineSR[chn]+" && "+invdphicut[chn]+" && "+metomeffcut[chn]
configMgr.cutsDict["CR1a"]  = baselineSR[chn]+" && "+dphicut[chn]+" && "+metomeffcut[chn]
...
#JES (tree-based)
jes = Systematic("JES","","_JESUP","_JESDOWN","tree","overallSys")
...
#bTagTop = Systematic("bTagTop", bTagWeights ,bTagSystWeightsUp,bTagSystWeightsDown,"weight","overallSys")
#bTagW = Systematic("bTagW",bTagWeights,bTagSystWeightsUp,bTagSystWeightsDown,"weight","overallSys")

bTagTop = Systematic("bTag", bTagWeights, bTagSystWeightsUp,   bTagSystWeightsDown, "weight", "overallSys")
bTagW   = Systematic("bTag", bTagWeights, bTagSystWeightsDown, bTagSystWeightsUp,   "weight", "overallSys")
...
for point in allpoints:
    ...
    #Channel
    if MYFIT!=FITTYPE.Background:
        SR = myFitConfig.addChannel("meffInc",["SR"],2,1400000,2100000)
