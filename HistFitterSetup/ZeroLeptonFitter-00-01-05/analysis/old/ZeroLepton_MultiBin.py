...
# set pickedSRs 0-2: 0 for Grid name, 1 for SR   
    
if pickedSRs!=None:  #pickedSRs is set by the "-r" HistFitter option
    grid=pickedSRs[0]

    print "Grid", pickedSRs[0],"oooooooooooooooooo", pickedSRs[1]
    if pickedSRs[1]=="SRA":chn=0
    if pickedSRs[1]=="SRAp":chn=1
    if pickedSRs[1]=="SRB":chn=2
    if pickedSRs[1]=="SRC":chn=3
    if pickedSRs[1]=="SRD":chn=4
    if pickedSRs[1]=="SRE":chn=5

    print pickedSRs[2],"oooooooooooooooooo"
    try:
        meff=int(pickedSRs[2])*1000
        theoSysTopNumber=0.20
        theoSysWNumber=0.20
        anaName=pickedSRs[1]+pickedSRs[2]
    except:
        meff=selections[pickedSRs[2]][chn]
        theoSysTopNumber=theoSysTopMap[pickedSRs[2]][chn]/100.
        theoSysWNumber=theoSysWMap[pickedSRs[2]][chn]/100.
        anaName=pickedSRs[1]+pickedSRs[2]
...
configMgr.analysisName = "ZLMB"+"_"+str(nbin)+"bin_"+anaName+"_"+grid+"_"+allpoints[0] #ATT
...
for point in allpoints:
    ...
    #-------------------------------------------------
    if MYFIT==FITTYPE.Exclusion:
        sigSample = Sample(grid+"_"+point,kRed)
    ...
    CRTop = myFitConfig.addChannel("meffInc",["CRTop"],nbin,minbin,maxbin) 
    CRTop.useOverflowBin=True 
    CRTop.useUnderflowBin=False 
    ...
    CRZ = myFitConfig.addChannel("meffInc",["CRZ"],nbin,minbin,maxbin)
    CRZ.useOverflowBin=True 
    CRZ.useUnderflowBin=False 
