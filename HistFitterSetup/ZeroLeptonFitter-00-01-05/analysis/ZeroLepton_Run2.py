...
if sigSamples != None:
    if "grid" in sigSamples[0]: # first entry allowed to specify a grid name
        ...
        if not os.path.exists(os.path.join(INPUTDIR_SIGNAL, grid+".root")):
...
if configMgr.readFromTree:
    ...
    # QCD
    qcdFiles.append(os.path.join(INPUTDIR, "QCD.root"))
    ...
    if zlFitterConfig.useFilteredNtuples:
        ...
        # Diboson
        dibosonFiles.append(os.path.join(INPUTDIR, "DibosonMassiveCB.root"))
  
        # Top
        topFiles.append(os.path.join(INPUTDIR, "Top.root"))
    ...
    #data
    dataFiles.append(os.path.join(INPUTDIR_DATA, "DataMain_Oct25.root"))
        ...
        for REGION in myFitConfig.channels:
            ...
            for sam in REGION.sampleList:                     
                ...
                #Top background
                    ...
                    sam.addSystematic(Systematic("TopPowhegHerwig", configMgr.weights, 1.+errorPowhegHerwig, 1-errorPowhegHerwig, "user", "userOverallSys"))                    
                    #radiation
