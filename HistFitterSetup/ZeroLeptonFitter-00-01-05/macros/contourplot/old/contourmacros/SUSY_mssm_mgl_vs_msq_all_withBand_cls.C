...
void SUSY_mssm_mgl_vs_msq_all_withBand_cls( TString fname0 = "Outputs/mssm_combined_fixSigXSecNominal__1_harvest_list.root",// nominal
                                      TString fname1 = "Outputs/mssm_combined_fixSigXSecUp__1_harvest_list.root",               // Up
                                      TString fname2 = "Outputs/mssm_combined_fixSigXSecDown__1_harvest_list.root",               // Down  
				      ...
				      TString hname6 = "sigp1ref")
{
  ...
  Int_t nsigma=2;
  TLegend *leg = new TLegend (0.64,0.52,0.90,0.68);
  ...
  if (discexcl==1) {
	...
	if(!blind){
	  ...
	  if (contour_obscls!=0) DrawContourLine95( leg, contour_obscls, "Observed limit", CombinationGlob::c_DarkRed, 1, 4);
        ...
        if(plotMoriond) leg->AddEntry(zl_obs,"7TeV (4.7fb^{-1}) Observed","f");
  ...
  TLatex *Leg0 = new TLatex( xmin, ymax + dy*0.025, Form("Squark-gluino-neutralino model, m(#tilde{#chi}_{1}^{0}) = %.0f GeV",lspmass) );
  ...
  Leg1->SetTextColor( 1 );
  Leg1->DrawLatex(0.65,0.79, Form("#int L dt = %1.1f fb^{-1},  #sqrt{s}=8 TeV",lumi));  // 0.32,0.87
  Leg1->AppendPad();
  ...
  leg3->DrawLatex(0.65, 0.71, Form("0-lepton combined"));   // 0.27,0.87
  leg3->AppendPad();
  ...
  atlasLabel->DrawLatex(0.65,0.87, "ATLAS"); // 0.15,0.87
  ...
  //prel->DrawLatex(0.77, 0.87, "for approval");   // 0.27,0.87
  //prel->DrawLatex(0.77, 0.87, "Preliminary");   // 0.27,0.87
