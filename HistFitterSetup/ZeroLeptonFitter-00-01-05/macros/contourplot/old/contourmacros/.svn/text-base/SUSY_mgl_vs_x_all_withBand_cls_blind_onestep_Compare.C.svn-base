...
void SUSY_mgl_vs_x_all_withBand_cls_blind_onestep_Compare( TString& fname01 = "mudat_list.root", TString& fname02 = "mudat_list.root", // nominal
        TString fname1 = "",               // Up
        TString fname2 = "",               // Down  
        ...
        TString hname6 = "sigp1ref")
{
    ...
    cout << "--- Reading root base file: " << fname01 << endl;
    TFile* f01 = TFile::Open( fname01, "READ" );
    TFile* f02 = TFile::Open( fname02, "READ" );

    if (!f01) {
        cout << "*** Error: could not retrieve histogram: " << hname0 << " in file: " << f01->GetName() 
    ...
    TH2F* histecls = (TH2F*)f01->Get( "sigp1expclsf" ); 
    TH2F* histocls = (TH2F*)f01->Get( "sigp1clsf" ); 
    if (histecls!=0) histecls->SetDirectory(0);
    if (histocls!=0) histocls->SetDirectory(0);
    
    TH2F* histecls2 = (TH2F*)f02->Get( "sigp1expclsf" ); 
    TH2F* histocls2 = (TH2F*)f02->Get( "sigp1clsf" ); 
    if (histecls2!=0) histecls2->SetDirectory(0);
    if (histocls2!=0) histocls2->SetDirectory(0);
    ...
    if(f6){
        ...
        if(fname01.Contains("GG_onestep")) SignalUncertainties->SetBranchAddress("mgluino",&mgluino);
        else if(fname01.Contains("SS_onestep")) SignalUncertainties->SetBranchAddress("msquark",&mgluino);
    ...
    TH2F* hist0 = (TH2F*)f01->Get( hname0 );
    TH2F* hist1 = (TH2F*)f01->Get( hname1 );
    TH2F* hist3 = (TH2F*)f01->Get( hname3 );
    TH2F* hist5 = (TH2F*)f01->Get( hname5 );
    TH2F* hist6 = (TH2F*)f01->Get( hname6 );
    ...
    TH2F* contour_em1s    = ( hist5!=0 ? FixAndSetBorders( *hist5, "contour_ep1s", "contour_ep1s", 0 ) : 0 );
    ...
    TH2F* contour_expcls2(0);
    if (histecls2!=0)     { contour_expcls2     = FixAndSetBorders( *histecls2, "contour_expcls2", "contour_expcls2", 0 ); }
    ...
    if (contour_ep1s) { 
        ...
        contour_ep1s->SetBinContent(2700, 800, 2.0);
        contour_em1s->SetBinContent(2700, 800, 2.0);
        contour_ep1s->SetBinContent(800, 2400, 2.0);
        contour_em1s->SetBinContent(800, 2400, 2.0);
        contour_ep1s->SetBinContent(800, 800, 2.0);
        contour_em1s->SetBinContent(800, 800, 2.0);
    }
    ...
    TString title;
    if(fname01.Contains("GG_onestep")) title=  "m_{#tilde{g}} vs x - ATLAS for approval";
    else if(fname01.Contains("SS_onestep")) title=  "m_{#tilde{q}} vs x - ATLAS for approval";
    float xlow=170., xhigh=1600., ylow=0., yhigh=1.5;
    if(fname01.Contains("SS_onestep")) xhigh= 1300.;
    ...
    if(fname01.Contains("GG_onestep"))   frame->SetXTitle( "gluino mass [GeV]" );
    else if(fname01.Contains("SS_onestep"))   frame->SetXTitle( "squark mass [GeV]" );
    //frame->SetXTitle( "gluino mass [GeV]" );
    if(fname01.Contains("GG_onestep"))   frame->SetYTitle( "x=#Deltam(#chi^{#pm},LSP)/#Deltam(#tilde{g},LSP)" );
    else if(fname01.Contains("SS_onestep"))   frame->SetYTitle( "x=#Deltam(#chi^{#pm},LSP)/#Deltam(#tilde{q},LSP)" );
    ...
    if(show7TeVlimits){
        c->cd();
        if (fname01.Contains("SS")) graph_7TeV= SimplifiedModels_7TeV("SM_SS_onestep_LSP60") ;
        else if (fname01.Contains("GG"))graph_7TeV= SimplifiedModels_7TeV("SM_GG_onestep_LSP60") ; 
    ...
    if(show7TeVlimits){
        c->cd();
        if (fname01.Contains("SS")) SimplifiedModels_7TeV("SM_SS_onestep_LSP60",false) ;
        else if (fname01.Contains("GG")) SimplifiedModels_7TeV("SM_GG_onestep_LSP60",false) ; 
    }
    ...
    if (discexcl==1) {
        ...
        if (!extExpectation) { // expectation from toys
            ...
            TLine* line1;
	    if(fname01.Contains("SS")) line1= new TLine( 222, 1.375, 272, 1.375);
            ...
            line1->Draw("SAME") ;
            TLine* line2;
	    if(fname01.Contains("SS")) line2= new TLine( 222, 1.315, 272, 1.315);
		...
		DrawContourLine95( leg, contour_expcls, "0-lepton standard SRs", c_myExp, 6 );
           }
            if (contour_expcls2!=0) {
                cout << "v 95% CLS" << endl;
                //DrawContourLine95( leg, contour_expcls, "exp. limit 95% CL", c_myExp, 6 ); 
		DrawContourLine95( leg, contour_expcls2, "0-lepton boosted W SRs", kGreen, 6 );
            }
    ...
    TString LegendTitle="";
    if(fname01.Contains("GG_onestep")) LegendTitle="Simplified model, #tilde{g}#tilde{g} #rightarrow q#bar{q}q#bar{q} #tilde{#chi}^{#pm}_{1}#tilde{#chi}^{#pm}_{1} #rightarrow q#bar{q}q#bar{q} W^{#pm}W^{#pm} #tilde{#chi}^{0}_{1}#tilde{#chi}^{0}_{1}"; 
    else if(fname01.Contains("SS_onestep")) LegendTitle="Simplified model, #tilde{q}#tilde{q}* #rightarrow q#bar{q} #tilde{#chi}^{+}_{1}#tilde{#chi}^{-}_{1} #rightarrow q#bar{q} W^{+}W^{-} #tilde{#chi}^{0}_{1}#tilde{#chi}^{0}_{1}" ; 
    ...
    if (prefix!=0) {
        ...
        Leg2->DrawLatex(0.16,0.75,"m(LSP)=60[GeV]");
        Leg2->AppendPad(); 
    }
    ...
    Show_UL(fname01, c2, !blind, xlow, xhigh, ylow, yhigh,false);

    //draw best SRs
    if(showSR){
        Show_SR(fname01, c, xlow, xhigh, ylow, yhigh, useShape);
    }
    ...
    TObjArray* arr = fname01.Tokenize("/");
        ...
        xhigh,float ylow,float yhigh, bool acceff=false)
{
    ...
    for( Int_t i = 0; i < tree->GetEntries(); i++ ){
        ...
        if(!(m1>500 && (m1-m2==25 || m2== m3+25 || m1-m2==50)) && UL*xsec<1000000.) lat.DrawLatex(m1,x,ULstring.Data());      
    ...
    TLatex *Leg0;
    if(oredList.Contains("SS_onestep")) Leg0 = new TLatex( xhigh+45, ylow+0.03*(yhigh-ylow), LegendTitle);
    else  Leg0 = new TLatex( xhigh+70, ylow+0.03*(yhigh-ylow), LegendTitle);
