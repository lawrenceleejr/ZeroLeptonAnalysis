...
#include "contourmacros/tevatron_sqgl_oldlim.C" 
#include "contourmacros/ATLAS10_1lepton.C"
#include "contourmacros/ATLAS_EPS_contours.C"

void SUSY_mgl_vs_msq_all_withBand_cls( TString fname0 = "mudat_list.root",// nominal
                                      TString fname1 = "",               // Up
                                      TString fname2 = "",               // Down  
                                      ...
                                      TString fnameMass= "../../../HistFitterUser/common/mSugraGridtanbeta10_gluinoSquarkMasses.root")
{
  ...
  if (true) { 
     ...
     for (int xbin = 1; xbin <= contour_ep1s->GetNbinsX(); xbin++) {
        for (int ybin = 1; ybin <= contour_ep1s->GetNbinsY(); ybin++) {
         if ( ax->GetBinCenter(xbin) > 800. && ax->GetBinCenter(xbin) < 960.
              && ay->GetBinCenter(ybin) >= 1800. && ay->GetBinCenter(ybin) <1950 ) {
  ...
  if (true) { 
     ...
     for (int xbin = 1; xbin <= contour_em1s->GetNbinsX(); xbin++) {
        for (int ybin = 1; ybin <= contour_em1s->GetNbinsY(); ybin++) {
           if (  ax->GetBinCenter(xbin) < 3000. && ay->GetBinCenter(ybin) >= 0.
                 && (ay->GetBinCenter(ybin) <1480 )){
              contour_em1s->SetBinContent(xbin, ybin, 2.0);  // NEEDED YU
           }
           if (ay->GetBinCenter(ybin)>1520 && ax->GetBinCenter(xbin)>1500)  {
              contour_em1s->SetBinContent(xbin, ybin, 1.0);  // NEEDED YU
           }
        }
     ...
     for (int xbin = 1; xbin <= contour_em1s->GetNbinsX(); xbin++) {
        for (int ybin = 1; ybin <= contour_em1s->GetNbinsY(); ybin++) {
           // island 1
           if ( ax->GetBinCenter(xbin) > 700. && ax->GetBinCenter(xbin) < 950.
                && ay->GetBinCenter(ybin) >= 2900. && ay->GetBinCenter(ybin) <3300 ) {
              /*
              ...
              */
              ...
              //if (contour_em1s->GetBinContent(xbin,ybin)<1.65) contour_em1s->SetBinContent(xbin, ybin, 2.0);
  ...
  if (true) { 
     ...
     for (int xbin = 1; xbin <= contour_ep1s->GetNbinsX(); xbin++) {
        for (int ybin = 1; ybin <= contour_ep1s->GetNbinsY(); ybin++) {
           ...
           if( (  ax->GetBinCenter(xbin) < 3000.
                 && ay->GetBinCenter(ybin) >= 0. && ay->GetBinCenter(ybin) <1350)
               ||
               (  ax->GetBinCenter(xbin) < 800.)
              ) {
              /*
              cout << "Found spot here: " << xbin << " (" << ax->GetBinCenter( xbin)  << "), "
                   << ybin << " (" << ay->GetBinCenter( ybin) << "), "
                   ...
                   cout << "   HACK : Setting above point by hand to 1.65 (!)" << endl;
              */
              //if (contour_ep1s->GetBinContent(xbin,ybin)<1.65) contour_ep1s->SetBinContent(xbin, ybin, 2.0);
              contour_ep1s->SetBinContent(xbin, ybin, 2.0); // NEEDED YU
           }
           if ( ax->GetBinCenter(xbin) > 700. && ax->GetBinCenter(xbin) < 870.
                && ay->GetBinCenter(ybin) >= 2900. && ay->GetBinCenter(ybin) <3300 ) {
              /*
              cout << "Found spot here: " << xbin << " (" << ax->GetBinCenter( xbin)  << "), "
                   << ybin << " (" << ay->GetBinCenter( ybin) << "), "
  ...
  // create and draw the frame
   //TH2F *frame = new TH2F("frame", "m_{0} vs m_{12} - ATLAS for approval", 100, 0., 1800. ,100, 0., 3200.);
   TH2F *frame; 
   if(showtevatron==1){
      frame = new TH2F("frame", "m_{0} vs m_{12} - ATLAS for approval", 100, 200., 1800. ,100, 200., 3000.);
   }else{
      frame = new TH2F("frame", "m_{0} vs m_{12} - ATLAS for approval", 100, 500., 1800. ,100, 500., 3000.);
   }
   ///TH2F *frame = new TH2F("frame", "m_{0} vs m_{12} - ATLAS for approval", 100, 200., 1800. ,100, 200., 3000.);
   //TH2F *frame = new TH2F("frame", "m_{0} vs m_{12} - ATLAS for approval", 100, 800., 1800. ,100, 800., 3000.);
  ...
  Int_t nsigma=2;
  TLegend *leg;
  leg = new TLegend (0.22,0.35,0.52,0.53);

  bool plot_stau=true;
  if(showtevatron==1 and plot_stau==false) leg = new TLegend (0.64,0.66,0.89,0.86);
  if(showtevatron==1 and plot_stau==true) leg = new TLegend (0.64,0.66,0.89,0.91);
  //if(showtevatron==1) leg = new TLegend (0.24,0.36,0.49,0.56);
  
  //TLegend *leg = new TLegend (0.64,0.6,0.92,0.8);
...
*/
  ...
  */
  ...
  if (true) { 
     ...
     for (int xbin = 1; xbin <= contour_obscls->GetNbinsX(); xbin++) {
        for (int ybin = 1; ybin <= contour_obscls->GetNbinsY(); ybin++) {
           if ( ( ax->GetBinCenter(xbin) > 700. && ax->GetBinCenter(xbin) < 940.
                  && ay->GetBinCenter(ybin) >= 2900. && ay->GetBinCenter(ybin) <3300 )
                || 
                ( ax->GetBinCenter(xbin) <3000 && ay->GetBinCenter(ybin) >= 0. && ay->GetBinCenter(ybin) <1470)
                ||
                ( ax->GetBinCenter(xbin) <800 )
              )
  ...
  if (true) { 
     ...
     for (int xbin = 1; xbin <= contour_expcls->GetNbinsX(); xbin++) {
        for (int ybin = 1; ybin <= contour_expcls->GetNbinsY(); ybin++) {
           if (  (ax->GetBinCenter(xbin) <3000 && ay->GetBinCenter(ybin) >= 0. && ay->GetBinCenter(ybin) <1390)
                 ||
                 ( ax->GetBinCenter(xbin) <800 )
              )
  ...
  if (true) { 
     ...
     for (int xbin = 1; xbin <= contour_esigxsm1s->GetNbinsX(); xbin++) {
        for (int ybin = 1; ybin <= contour_esigxsm1s->GetNbinsY(); ybin++) {
           if(
              (ax->GetBinCenter(xbin)>1520 && ax->GetBinCenter(xbin)<=1600 && ay->GetBinCenter(ybin)>1450 && ay->GetBinCenter(ybin)<=1520   ) 
              ){
     ...
     for (int xbin = 1; xbin <= contour_esigxsm1s->GetNbinsX(); xbin++) {
        for (int ybin = 1; ybin <= contour_esigxsm1s->GetNbinsY(); ybin++) {
           if(
              (ax->GetBinCenter(xbin) <3000 && ay->GetBinCenter(ybin) >= 0. && ay->GetBinCenter(ybin) <1380 )
              ||
              ( ax->GetBinCenter(xbin) <800 )
              //|| (ax->GetBinCenter(xbin)>1520 && ax->GetBinCenter(xbin)<=1600 && ay->GetBinCenter(ybin)>1400 && ay->GetBinCenter(ybin)<=1520   ) 
              ){
              contour_esigxsm1s->SetBinContent(xbin, ybin, 2.0);
           }
        }
  ...
  if (true) { 
     ...
     for (int xbin = 1; xbin <= contour_esigxsp1s->GetNbinsX(); xbin++) {
           for (int ybin = 1; ybin <= contour_esigxsp1s->GetNbinsY(); ybin++) {
              if(( ax->GetBinCenter(xbin) <3000 && ay->GetBinCenter(ybin) >= 0. && ay->GetBinCenter(ybin) <1500)
                 || ( ax->GetBinCenter(xbin) <800 ))
              {
        ...
        for (int ybin = 1; ybin <= contour_expcls->GetNbinsY(); ybin++) {
           if ( ax->GetBinCenter(xbin) > 700. && ax->GetBinCenter(xbin) < 870.
            ...
            cout << "Found spot here: " << xbin << " (" << ax->GetBinCenter( xbin)  << "), "
                 ...
                 cout << "   HACK : Setting above point by hand to 1.65 (!)" << endl;
              */
            //if (contour_expcls->GetBinContent(xbin,ybin)<1.65) contour_expcls->SetBinContent(xbin, ybin, 2.0);
            contour_expcls->SetBinContent(xbin, ybin, 2.0);
           }
        }
     }
  ...
  TGraph* grshadeExp = DrawExpectedBand_new( gr_contour_ep1s, gr_contour_em1s, c_myYellow, 1001, 800, 1800, 1000, 3000, 800, 900)->Clone();
  ...
  if (discexcl==1) {
       if (!extExpectation) { // expectation from toys
        ...
        if (showfixSigXSecBand) {
           ...
           TLine* line0;
           line0 = new TLine( 1428, 1660, 1509, 1660);

           line0->SetLineColor(c_myYellow);
           line0->SetLineWidth(10);
           //line0->Draw("SAME") ;
           
           TLine* line1;
           if(plot_stau) {
              line1 = new TLine( 1226, 2870, 1307, 2870);
           }else{
              line1 = new TLine( 1226, 2690, 1307, 2690);
           }
           //TLine* line1  = new TLine( 430, 1665, 509, 1665);
           ...
           TLine* line2;
           if(plot_stau){
              line2= new TLine( 1226, 2710, 1307, 2710);
           }else{
              line2= new TLine( 1226, 2528, 1307, 2528);
           }
           //TLine* line2  = new TLine( 435, 1510, 514, 1510);
  ...
  TLegend* tevatron;
  if(showtevatron==1) sqgl_oldlim(1800., 3000.);
  ...
  TGraph*  msugra_stau=msugra_stauLSP_mglmsq();
  if(plot_stau){
     msugra_stau->Draw("FSAME");
     msugra_stau->Draw("LSAME");
  }
  TGraph* msugra_noEWSB=msugraThExcl_mglmsq();
  msugra_noEWSB->SetFillColor( kGreen+2 );
  ...
  leg->AddEntry( msugra_noEWSB, "Theoretically excluded","F" );
  if(plot_stau) leg->AddEntry( msugra_stau, "Stau LSP","F" );
  ...
  Leg1->SetTextColor( 1 );
  if(showtevatron) {
     Leg1->DrawLatex(0.22,0.65, Form("#int L dt = %1.1f fb^{-1},  #sqrt{s}=8 TeV",lumi));  // 0.32,0.87
  }else{
     Leg1->DrawLatex(0.22,0.6, Form("#int L dt = %1.1f fb^{-1},  #sqrt{s}=8 TeV",lumi));  // 0.32,0.87
  }
  Leg1->AppendPad();
  ...
  if (prefix!=0){
     if(showtevatron) {
        Leg2->DrawLatex(0.22,0.58,prefix); // 0.15,0.81
        // Leg2->DrawLatex(0.22,0.51,"All limits are at 95 % CL_{s}"); // 0.15,0.81
     }else{
        Leg2->DrawLatex(0.22,0.55,prefix); // 0.15,0.81
     }
    Leg2->AppendPad(); 
  }
  ...
  if(showtevatron==1){
     atlasLabel->DrawLatex(0.34,0.78, "ATLAS"); // 0.15,0.87
  }else{
     atlasLabel->DrawLatex(0.3,0.75, "ATLAS"); // 0.15,0.87    
  }
  ...
  CombinationGlob::imgconv( c, Form("plots/atlascls_mglmsq_%s",outfile.Data()) );   
  ...
  if(showtevatron){
     //prel->DrawLatex(0.31, 0.73, "for approval");   // 0.27,0.87
     prel->DrawLatex(0.31, 0.73, "Preliminary");   
  }else{
     //prel->DrawLatex(0.3, 0.69, "for approval");   // 0.27,0.87
     prel->DrawLatex(0.3, 0.69, "Preliminary"); 
  }
  prel->AppendPad();

  TString prefixsave = TString(prefix).ReplaceAll(" ","_") + Form("%dinvfb_",lumi) + Form("wband%d_",showOneSigmaExpBand);
  CombinationGlob::imgconv( c, Form("plots/mglmsqcls_%s",outfile.Data()) );   
...
ContourGraph( TH2F* hist)
{
   ...
   h->GetXaxis()->SetRangeUser(800,3200);
...
TGraph* DrawExpectedBand_new(TGraph* gr1,  TGraph* gr2, Int_t fillColor, Int_t fillStyle,
   ...
   grshade->SetMarkerSize(2);
   grshade->Draw("F");
   
   int Nshade = grshade->GetN();
   cout<<" nshade "<<Nshade<<endl;
