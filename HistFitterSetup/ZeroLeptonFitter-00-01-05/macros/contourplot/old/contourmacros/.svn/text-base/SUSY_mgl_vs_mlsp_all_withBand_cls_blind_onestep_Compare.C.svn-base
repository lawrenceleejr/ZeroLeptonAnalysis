...
void SUSY_mgl_vs_mlsp_all_withBand_cls_blind_onestep_Compare( TString& fname01 = "mudat_list.root", TString& fname02 = "mudat_list.root",// nominal
        TString fname1 = "",               // Up
        TString fname2 = "",               // Down  
        ...
        TString hname6 = "sigp1ref")
{
    ...
    cout << "--- Reading root base file: " << fname02 << endl;
    TFile* f02 = TFile::Open( fname02, "READ" );
    if (!f02) {
        cout << "*** Error: could not retrieve histogram: " << hname0 << " in file: " <<f02->GetName() 
            << " ==> abort macro execution" << endl;
        return;
    }
    ...
    TH2F* histUL = (TH2F*)f01->Get( "upperLimit" );
    ...
    TH2F* contour_ep1s    = ( hist3!=0 ? FixAndSetBorders( *hist3, "contour_ep1s", "contour", 0 ) : 0 );
    TH2F* contour_em1s    = ( hist5!=0 ? FixAndSetBorders( *hist5, "contour_em1s", "contour", 0 ) : 0 );
    ...
    TString title;
    if(fname01.Contains("GG_onestep")) title=  "m_{#tilde{g}} vs m_{lsp} - ATLAS for approval";
    else if(fname01.Contains("SS_onestep")) title=  "m_{#tilde{q}} vs m_{lsp} - ATLAS for approval";
    float xlow = 0;
    float xhigh = 1600;
    float ylow = 0;
    float yhigh = 1600;
    if(fname01.Contains("SS_onestep")) {
      xhigh= 1250.;
      yhigh= 1250.;
    } 
...
if(show7TeVlimits){
    c->cd();
    if (fname01.Contains("SS")) graph_7TeV= SimplifiedModels_7TeV("SM_SS_onestep_X05") ;
    else if (fname01.Contains("GG"))graph_7TeV= SimplifiedModels_7TeV("SM_GG_onestep_X05") ; 
...
if(show7TeVlimits){
    c->cd();
    if (fname01.Contains("SS")) SimplifiedModels_7TeV("SM_SS_onestep_X05",false) ;
    else if (fname01.Contains("GG")) SimplifiedModels_7TeV("SM_GG_onestep_X05",false) ; 
}
...
if (discexcl==1) {
    ...
    if (!extExpectation) { // expectation from toys
        ...
        TLine* line1;
	if(fname01.Contains("SS")) line1= new TLine( 57, 1141, 106, 1141);
        else line1= new TLine( 75, 1465, 138, 1465);
        line1->SetLineWidth(2);
        line1->SetLineColor(CombinationGlob::c_DarkRed);
        line1->SetLineStyle(3);
        line1->Draw("SAME") ;
        TLine* line2;
	if(fname01.Contains("SS")) line2= new TLine( 57, 1100, 106, 1100);
	else line2= new TLine( 75, 1405, 138, 1405);
...
if (prefix!=0) {
    ...
    if (fname01.Contains("SS")) Leg2->DrawLatex(0.16,0.75,"m(#tilde{#chi}^{#pm})= (m(#tilde{q}) + m(LSP))/2");
    else Leg2->DrawLatex(0.16,0.75,"m(#tilde{#chi}^{#pm})= (m(#tilde{g}) + m(LSP))/2");
    Leg2->AppendPad(); 
}
...
if(yhigh > 2) {
    ...
    TString badLSPstr = "";
    if (fname01.Contains("SS")) badLSPstr = "#tilde{q} LSP";
    else if (fname01.Contains("GG")) badLSPstr = "#tilde{g} LSP";
...
void Show_SR(TString oredList,  TCanvas *can, float xlow, float xhigh, float ylow, float yhigh, bool useShape)
{
    ...
    for( Int_t i = 0; i < tree->GetEntries(); i++ ){
       ...
       if (oredList.Contains("SS_onestep") && m3>800) continue; 

       if(oredList.Contains("GG_onestep") && m1==1065 && m2==785 && m3==505) marker.DrawMarker(m1,m3);
       if(oredList.Contains("GG_onestep") && m1==1265 && m2==865 && m3==465) marker.DrawMarker(m1,m3);
       
       if (fID > 0)
            cout << "  At x = " << m1<< ", y = " << m3<< ", fID = " <<  fID<< endl;
	...
	sprintf(buffer,"%i",fID);
		...
		if(m1==315 && m3==235) right_position= false;	
                //if (right_position) lat.DrawLatex(m1,m3,mySR.Data());
                if (right_position) lat.DrawLatex(m1,m3,buffer);
            }     
        } else { // X on the Y axis
            if(m1 > xlow+50 &&
                    m1 < xhigh-50 &&
                    m3 > ylow+0.05 &&
                    m3 < yhigh-0.05 &&
                    m1 - (60+m3*(m1-60)) > 45 // Prevent scrunching of labels
              ) lat.DrawLatex(m1,m3,mySR.Data());
        }
    }
}
        ...
        float ylow, float yhigh, bool acceff=false)
{
	...
	if(oredList.Contains("GG_onestep") && m1==1265 && m2==865 && m3==465) marker.DrawMarker(m1,m3);
		...
		if(m1==315 && m3==235) right_position= false;
                if (right_position && UL*xsec<1000000.) lat.DrawLatex(m1,m3,ULstring.Data());
            }     
        }
