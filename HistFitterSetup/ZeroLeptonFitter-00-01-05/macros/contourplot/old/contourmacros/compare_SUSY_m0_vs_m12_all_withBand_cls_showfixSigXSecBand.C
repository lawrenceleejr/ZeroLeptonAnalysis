...
void compare_SUSY_m0_vs_m12_all_withBand_cls_showfixSigXSecBand( TString fname0="", TString fname1="",
                                                                 TString fname2="", TString fname3="", TString fname4="",
                                                                 ...
                                                                 TString fnameMass= "contourmacros/mSugraGridtanbeta10_gluinoSquarkMasses.root")
{
   ...
   if(showfixSigXSecBand){
      ...
      cout << "--- Reading root base file: " << fname3 << endl;
      f3 = TFile::Open( fname3, "READ" );
      cout << "--- Reading root base file: " << fname4 << endl;
      f4 = TFile::Open( fname4, "READ" );

      if(!f1 || !f2 || !f3 || !f4){
         cout << "*** Error: could not open in files: " << f1->GetName() <<" or "<< f2->GetName()
              << " " << f3->GetName() <<" or "<< f4->GetName() 
              << " ==> abort macro execution" << endl;
         return;
      }
   }
   ...
   TH2F* histecls_1 = (TH2F*)f1->Get( "sigp1expclsf" );
   TH2F* histecls_2 = (TH2F*)f2->Get( "sigp1expclsf" ); 
   TH2F* histecls_3 = (TH2F*)f3->Get( "sigp1expclsf" );
   TH2F* histecls_4 = (TH2F*)f4->Get( "sigp1expclsf" ); 
   ...
   TH2F* contour_expcls_1(0);
   if (histecls_1!=0)     { contour_expcls_1     = FixAndSetBorders( *histecls_1, "contour_expcls_1", "contour_expcls_1", 0 ); }
   TH2F* contour_expcls_2(0);
   if (histecls_2!=0)     { contour_expcls_2     = FixAndSetBorders( *histecls_2, "contour_expcls_2", "contour_expcls_2", 0 ); }
   TH2F* contour_expcls_3(0);
   if (histecls_3!=0)     { contour_expcls_3     = FixAndSetBorders( *histecls_3, "contour_expcls_3", "contour_expcls_3", 0 ); }
   TH2F* contour_expcls_4(0);
   if (histecls_4!=0)     { contour_expcls_4     = FixAndSetBorders( *histecls_4, "contour_expcls_4", "contour_expcls_4", 0 ); }
  ...
  // create and draw the frame
   TH2F *frame = new TH2F("frame", "m_{0} vs m_{12} - ATLAS work in progress", 100, 125., 2500., 100, 200., 750. );
   ...
   TLegend *leg = new TLegend(0.59,0.7,0.87,0.915);
   ...
   if (discexcl==1) {
      ...
      if (!extExpectation) { 
         if (showfixSigXSecBand) {
            //Turn off for illustration
            //if (contour_esigxsp1s!=0) DrawContourLine95( leg, contour_esigxsp1s, "", CombinationGlob::c_DarkOrange, 3, 2 );
         }
         
         // Turn off observation (blind)
         if (contour_obscls!=0) ////DrawContourLine95( leg, contour_obscls, "Observed limit (#pm1 #sigma^{SUSY}_{theory})", CombinationGlob::c_DarkRed, 1, 4);
            if (showfixSigXSecBand) {
               //Turn off for illustration
               //if (contour_esigxsm1s!=0) DrawContourLine95( leg, contour_esigxsm1s, "", CombinationGlob::c_DarkOrange, 3, 2 );
            }

         // Compare the expected limits!
         if (contour_expcls!=0) DrawContourLine95( leg, contour_expcls, fname0, CombinationGlob::c_DarkOrange, 6 );
         if (contour_expcls_1!=0) DrawContourLine95( leg, contour_expcls_1, fname1, CombinationGlob::c_DarkGreen, 6 );
         if (contour_expcls_2!=0) DrawContourLine95( leg, contour_expcls_2, fname2, CombinationGlob::c_DarkGray, 6 );
         if (contour_expcls_3!=0) DrawContourLine95( leg, contour_expcls_3, fname3, CombinationGlob::c_DarkPink, 6 ); 
         if (contour_expcls_4!=0) DrawContourLine95( leg, contour_expcls_4, fname4, CombinationGlob::c_DarkRed, 6 );
  ...
  //dont forget to uncomment line 478
  ...
  //uncomment if you uncomment leg2 above
  ...
  TString outfile = TString(Form("%1.2finvfb_",lumi)) + TString(Form("wband%d_",showOneSigmaExpBand)) + TString(Form("wfixSigXSecband%d_",showfixSigXSecBand)) + TString(Form("showcms%d_",showcms)) + objstring->GetString().ReplaceAll(".root","");
  delete arr;
...
DrawContourLine95( TLegend *leg, TH2F* hist, const TString& text="", Int_t linecolor=CombinationGlob::c_VDarkGray, Int_t linestyle=2, Int_t linewidth=2 )
{
  ...
  if (!text.IsNull()) leg->AddEntry(h,TString(text.Data()).ReplaceAll("Output_","").ReplaceAll("hypotest__1_harvest_list","").ReplaceAll("_.root","").ReplaceAll("_merged","").ReplaceAll("medium_meff","_meff").ReplaceAll("medium_nBJet","_nBJet"),"l"); 
}
