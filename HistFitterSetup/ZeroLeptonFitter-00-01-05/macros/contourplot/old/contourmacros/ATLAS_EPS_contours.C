...
TGraph* msugra_noEWSB(TString noewsbfile) {
  ...
  g_exc_noEWSB->SetFillColor(kMagenta-10);
...
TGraph* msugra_lepchrg(TString chrgfile) {
  ...
  g_exc_lepc->SetPoint(npts+2,4200,0);
  g_exc_lepc->SetPoint(npts+3,4200,y0);
  g_exc_lepc->SetLineColor(kGray+1);
  g_exc_lepc->SetLineWidth(1);
  g_exc_lepc->SetFillColor(kRed-9);
...
TGraph *msugra_stauLSP() {
  ...
  g_exc_stauLSP->SetFillColor(kMagenta-10);
