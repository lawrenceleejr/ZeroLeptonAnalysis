...
#INPUTDIR_SIGNALDB = "/afs/cern.ch/atlas/groups/susy/0lepton/ZeroLeptonFitter/ZeroLepton-00-00-42_Light/"
#INPUTDIR_SIGNALDB = "/afs/cern.ch/user/m/mamuzic/public/v53/SM_TT_directCC/"
#INPUTDIR_SIGNALDB = "/afs/cern.ch/user/m/mamuzic/public/v53/SM_TT_directCC_nogfe/"
#INPUTDIR_SIGNALDB = "/afs/cern.ch/user/m/mamuzic/public/v53/"
#INPUTDIR_SIGNALDB = "/home/tj1008/ZeroLepton-00-00-51/ZeroLepton/Tools/"

INPUTDIR_SIGNALDB = "/afs/cern.ch/user/m/marijam/public/SignalDBs/"

if socket.getfqdn().find("nikhef") != -1:
    INPUTDIR_SIGNALDB = "/glusterfs/atlas4/users/gbesjes/ZeroLepton-00-00-53_Light/" 
if socket.getfqdn().find("bfg") != -1:
    INPUTDIR_SIGNALDB = "/storage/groups/atlher/SUSY/ZeroLeptonFactory/smallntuple/8TeV/ZeroLepton-00-00-53/LightNtuples/" 
...
#from MyAnaList_MUED import *
#from MyAnaList_onestepPaper2013 import *
from MyAnaList_Shape import *
from MyAnaList_Gluino_Stop_charm import *
from MyAnaList_pMSSM import *
...
OUTPUTDIR = "Outputs/"
PLOTSDIR = "plots/"
OUTPUTDIR1 = "Outputs_SS_Boo/"
OUTPUTDIR2 = "Outputs_SS_noBoo/"
...
def parseCmdLine(args):
    ...
    parser.add_option("--lhcp13", dest="lhcp13", help="load analyses for lhcp '13", default=False, action="store_true")
    parser.add_option("--compareContours", dest="compareContours", help="use if you want to compare conture with user-set contour", action='store_true', default=False)
    ...
    if readDir:
        ...
        #for name in filenames:
            #print name
                ...
                #extraNames += os.listdir(name)

        #print extraNames
        #filenames = []
        #for (dirpath, dirnames, fnames) in os.walk(inputdir):
...
def MergeFiles(config):
    ...
    print config.grid,":",grid_name,",",name_match,",",name_filter
    #print "extra match: %s, extra filter: %s" % (extra_match, extra_filter)
    ...
    for ana in config.anaList:
        ...
        for filename in filenames:
            ...
            #if extra_match != "" and filename.find(extra_match) == -1:
                #continue
            
            #if extra_filter != "" and filename.find(extra_filter) != -1: 
                #continue
...
def getDSIDs(config):
    ...
    try:
        ...
        cmd="python $ZEROLEPTONFITTER/ToolKit/makeSignalPointPickle2012.py"
        print cmd
        subprocess.call(cmd, shell=True)
        picklefile = open('signalPointPickle2012.pkl','rb')    
        pass
    ...
    for ana in config.anaList:        
        ...
        for xs in allXS:
            ...
            if config.grid.find("SM_GG_onestep")!=-1:
                ...
                interpretation = "mgluino:mchargino:mlsp"; 
#                basename = INPUTDIR+config.outputName+"_"+ana+"_fixSigXSec"+xs
...
##             ("v",900,0,550)
    ...
    for aLine in allLines:
        ...
        for ana in config.anaList:
            ...
            for text in textfile.readlines():
                ...
                if line=="d":
                    ...
                    if m12 != cut:
                        print "%s m12 cut failed" % ana
                        continue
                    var=m0
                else:
                    if m0 != cut:
                        print "%s m0 cut failed: m0=%.2f cut=%.2f" % (ana, m0, cut)
                        continue
                    var = m12
                    #print UL,m12,ana

                print "%s point passed cuts; checking ranges" % ana

                if var<MASSMIN:
                    print "%s: var %.2f lower than min " % (ana, var)
                    continue      
                if var>MASSMAX: 
                    print "%s: var %.2f higher than max " % (ana, var)
                    continue
                
                print "ranges pass; data: %.2f UL=%.2f ana=%s" % (var, UL, ana)

                #print ana,"==========="
                isPaper13=False
                if ana in [AnaConvert(tmp) for tmp in anaInvDictPaper13.keys()]: 
        ...
        if g:
            g.Draw("AP")
            leg.AddEntry(g,"Best all analysis","P")
...
def MakePlots(config):
    ...
    #leg= TLegend(0.5,0.8,0.90,0.89)
    ###xleg.SetTextSize( CombinationGlob::DescriptionTextSize );
    leg.SetTextSize( 0.03 );
    leg.SetTextFont( 42 );
    leg.SetFillColor( 0 );
    leg.SetFillStyle(1001);
    ...
    #for ana in ["combined_cutcount", "combined_5bin"]: #combine multiple by hand
        ...
        canvas.cd(0)
        h = DrawContourLine95( leg, contour, legName, color, linestyle, linewidth );
        leg.Draw("same")
        ...
        if config.makeUL:
            ...
            local_canvas.Print(PLOTSDIR+"/"+config.outputName+"_"+ana+".gif")
    ...
    canvas.Print(PLOTSDIR+"/"+config.outputName+".gif")
...
def MakeATLASPlots(config) :
    ...
    if config.compareContours:
       print "%s %s are two Outputs which you decided to use, change it in case you need" % (OUTPUTDIR1,OUTPUTDIR2)
       combined_filename1 = OUTPUTDIR1+config.outputName+"_combined_fixSigXSecNominal"+listSuffix+".root"
       combined_filename2 = OUTPUTDIR2+config.outputName+"_combined_fixSigXSecNominal"+listSuffix+".root" 
    ...
    elif "SM_SS_direct" in config.grid or config.grid == "SM_GG_direct" or config.grid == "SM_SG_direct":
    ...
    if config.compareContours:
       if config.grid == "SM_GG_onestep_LSP60" or config.grid == "SM_SS_onestep_LSP60": 
           plotMacro = "makecontourplots_Compare_onestep_LSP60.C"
       elif config.grid == "SM_GG_onestep_X05" or config.grid == "SM_SS_onestep_X05": 
           plotMacro = "makecontourplots_Compare_onestep_X05.C"
    ...
    if config.compareContours:
        try:
           print "processing compareContours"
           cmd="root -b -q \"%s(\\\"%s\\\",\\\"%s\\\", \"%s\")\"" % (plotMacro, combined_filename1, combined_filename2, str(config.shape).lower())
        except:
           print "ERROR: fix script: %s" % plotMacro
...
def main():
    ...
    if config.opti and not config.shape:
        anaList += anaListPaper13 #add Paper 13 regions when optimising  

    if not config.lhcp13 and not config.opti and not config.shape:
        anaList += anaListPaper13
    elif not config.opti and config.shape:
        anaList = anaListPaper13Shape
    ...
    # LHCP '13 naming does not have Wres and Wunres
    if not config.lhcp13: 
        config.anaList=[AnaConvert(ana) for ana in anaList]
    else:
        description = ["chan","meff","metomeff","met","jet1pt","jet2pt","jet3pt","jet4pt","jet5pt","jet6pt","metSig","dPhi"]
        config.anaList=[AnaConvert(ana, description) for ana in anaList]

    ## TODO: hack, AnaList for GG and SG missing the two W parameters
    #if config.grid == "SM_GG_direct" or config.grid == "SM_SG_direct":
        #description = ["chan","meff","metomeff","met","jet1pt","jet2pt","jet3pt","jet4pt","jet5pt","jet6pt","metSig","dPhi"]
        #config.anaList=[AnaConvert(ana, description) for ana in anaList]
    #else:
        #config.anaList=[AnaConvert(ana) for ana in anaList]
    ...
    if config.makeATLASplots or config.doAll:
