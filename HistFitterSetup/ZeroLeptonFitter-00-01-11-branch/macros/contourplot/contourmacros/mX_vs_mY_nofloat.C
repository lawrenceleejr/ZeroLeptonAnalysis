...
*/
const char*
mX_vs_mY_nofloat(const char* textfile = 0, TH2D* inputHist = 0, const char* rootfile = "mXmY_nofloat.root", TString id1="mX",TString id2="mY", int   nbinsX=21,int nbinsY=17, float minX=20,float maxX=860, float minY=92.5, float maxY=347.5)
{
   ...
   if (inputHist!=NULL){
     ...
     p1clsf = DrawUtil::triwsmooth( tree, "CLs:m12:m0", "p1clsf" , "Observed CLs", "p1>0 && p1<=1", clonep1clsf );}
   else{
     p1clsf = DrawUtil::triwsmooth( tree, "CLs:m12:m0", "p1clsf" , "Observed CLs", "p1>0 && p1<=1", inputHist );
   }
