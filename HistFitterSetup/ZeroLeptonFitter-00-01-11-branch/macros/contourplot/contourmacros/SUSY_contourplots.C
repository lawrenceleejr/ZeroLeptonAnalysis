...
DrawContourLine95( TLegend *leg, TH2F* hist, const TString& text="", Int_t linecolor=CombinationGlob::c_VDarkGray, Int_t linestyle=2, Int_t linewidth=2 , TString Grid="" )
{
  ...
  cout << "signif: " <<StatTools::GetSigma(pval) << endl;
  h->SetContourLevel( 0, signif );
...
TString GetSRName(int fID, TString txtfile){
  ...
  */
  ...
  // // text = text.ReplaceAll("SRJigsawSR","");
  // text = text.ReplaceAll("Jigsaw","");
    ...
    TFile* customContourFile=0)
{
  ...
  // TString disc_p1obs = "upperLimit";

  // hist for best SR contour expected
  TH2F* histecls0 = (TH2F*)f0->Get(disc_p0exp);
  // place holder for xsec +/-1 lines
  TH2F* histecls1 = (TH2F*)f0->Get("sigclsu1s");  
  TH2F* histecls2 = (TH2F*)f0->Get("sigclsd1s");
  TH2F* histecls0zz= (TH2F*)f0->Get(disc_p1obs);


  cout << histecls0->GetName()     << endl;
  cout << histecls1->GetName()     << endl;
  cout << histecls2->GetName()     << endl;
  cout << histecls0zz->GetName()    << endl;
  ...
  TH2F* contourexp_comb(0);
  if (histecls0!=0) { contourexp_comb     = FixAndSetBorders( *histecls0, "contourexp_comb", "contourexp_comb", 0 ); }
