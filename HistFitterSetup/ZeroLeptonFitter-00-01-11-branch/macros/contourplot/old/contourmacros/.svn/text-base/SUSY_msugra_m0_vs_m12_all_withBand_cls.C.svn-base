...
void SUSY_msugra_m0_vs_m12_all_withBand_cls( TString fname0 = "Outputs/msugra_30_P_combined_fixSigXSecNominal__1_harvest_list.root",// nominal
                                      ...
                                      TString fnameMass2= "common/mSugraGridtanbeta30_charginoMasses.root")
{
  ...
  msugraThExcl("common/msugra_status_tanb30.txt", staulsp, negmasssq, noRGE, noEWSB, tachyon,fnameMass2);
  ...
  float xlow=100., xhigh=6000., ylow=300., yhigh=950.;
  TH2F *frame = new TH2F("frame", "m_{0} vs m_{12} - ATLAS work in progress", 100, xlow, xhigh, 100, ylow, yhigh );
  ...
  Int_t nsigma=2;
  TLegend *leg = new TLegend(0.62,0.47,0.92,0.665);
  ...
  // squark, gluino, higgs mass contours 
  Add_MassContour(fnameMass,fnameMass2);
...
ContourGraph( TH2F* hist)
{
   ...
   h->GetYaxis()->SetRangeUser(250,1200);
...
void Add_MassContour(TString fnameMass, TString fnameMass2)
{
  ...
  TFile* f5 = TFile::Open( fnameMass2, "READ" );
  TH2F* histHiggs = (TH2F*)f5->Get( "mSugraGrid_higgsMasses" );
  histHiggs->SetDirectory(0);
  f5->Close();
  ...
  TH2F* histHiggsMass    = FixAndSetBorders( *histHiggs, "HiggsMass", "HiggsMass", 10000 );

  colH = CombinationGlob::c_LightPink; 
  colS = 16; 
  DrawContourMassLine( histHiggsMass, 114.0 ,colH, colS);
  DrawContourMassLine( histHiggsMass, 115.0 ,colH, colS);
  DrawContourMassLine( histHiggsMass, 116.0 ,colH, colS);
  DrawContourMassLine( histHiggsMass, 117.0 ,colH, colS);
  DrawContourMassLine( histHiggsMass, 118.0 ,colH, colS);
  DrawContourMassLine( histHiggsMass, 119.0 ,colH, colS);
  DrawContourMassLine( histHiggsMass, 120.0 ,colH, colS);
  DrawContourMassLine( histHiggsMass, 121.0 ,colH, colS);
  DrawContourMassLine( histHiggsMass, 122.0 ,colH, colS);
  DrawContourMassLine( histHiggsMass, 123.0 ,colH, colS);
  DrawContourMassLine( histHiggsMass, 124.0 ,colH, colS);
  DrawContourMassLine( histHiggsMass, 125.0 ,colH, colS);
  DrawContourMassLine( histHiggsMass, 126.0 ,colH, colS);
  DrawContourMassLine( histHiggsMass, 127.0 ,colH, colS);
  DrawContourMassLine( histHiggsMass, 128.0 ,colH, colS);
  DrawContourMassLine( histHiggsMass, 129.0 ,colH, colS);
  DrawContourMassLine( histHiggsMass, 130.0 ,colH, colS);
  ...
  DrawContourMassLine( histSquarkMass, 7000.0 );
  DrawContourMassLine( histSquarkMass, 7400.0 );
  ...
  DrawContourMassLine( histGluinoMass, 2400.0 );
  DrawContourMassLine( histGluinoMass, 2600.0 );
  DrawContourMassLine( histGluinoMass, 2800.0 );
  DrawContourMassLine( histGluinoMass, 3000.0 );

  TLatex * s1400 = new TLatex( 780, 595, "#tilde{q} (1400 GeV)" );
  s1400->SetTextAlign( 11 );
  s1400->SetTextAngle(-70);
  s1400->SetTextSize( 0.025 );
  s1400->SetTextColor( 16 );
  s1400->Draw();   

  TLatex * s1800 = new TLatex( 1030, 790, "#tilde{q} (1800 GeV)" );
  ...
  TLatex * s2200 = new TLatex( 1400, 900, "#tilde{q} (2200 GeV)" );
  s2200->SetTextAlign( 11 );
  s2200->SetTextAngle(-70);
  ...
  TLatex * g1600 = new TLatex( 1750, 680, "#tilde{g} (1600 GeV)" );
  ...
  TLatex * g1800 = new TLatex( 1950, 772, "#tilde{g} (1800 GeV)" );
  g1800->SetTextAlign( 11 );
  g1800->SetTextAngle(-6);
  g1800->SetTextSize( 0.025 );
  g1800->SetTextColor( 16 );
  g1800->Draw();

  TLatex * h124 = new TLatex( 1600, 870, "H (124 GeV)" );
  h124->SetTextAlign( 11 );
  h124->SetTextAngle(-80);
  h124->SetTextSize( 0.025 );
  h124->SetTextColor( colH );
  h124->Draw();

  TLatex * h125 = new TLatex( 2050, 870, "H (125 GeV)" );
  h125->SetTextAlign( 11 );
  h125->SetTextAngle(-80);
  h125->SetTextSize( 0.025 );
  h125->SetTextColor( colH );
  h125->Draw();

  TLatex * h126 = new TLatex( 2750, 870, "H (126 GeV)" );
  h126->SetTextAlign( 11 );
  h126->SetTextAngle(-80);
  h126->SetTextSize( 0.025 );
  h126->SetTextColor( colH );
  h126->Draw();
}
...
void Show_SR(TString oredList, TCanvas *can, float xlow, float xhigh, float ylow, float yhigh, bool useShape)
{
    ...
    for( Int_t i = 0; i < tree->GetEntries(); i++ ){
        ...
        if( m0>3900 && m12>550) continue; // remove legend region
...
RemoveIsland( TH2F* hist, Int_t type=0)
{
  ...
  for(int i=0; i<hist->GetNbinsX(); i++)
    for(int j=0; j<hist->GetNbinsY(); j++){
      ...
      if(m0<1300 && m12<500) hist->SetBinContent(i+1,j+1,3.); // small mass corner
    }
}
