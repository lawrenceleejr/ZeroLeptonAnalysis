...
void SUSY_SM_TT_directCC_m0_vs_m12_all_withBand_cls( TString fname0="", TString fname1="",
        TString fname2="",
        TString gridname="",
        ...
        TString fnameMass= "")
{
    ...
    double plotrange=340.;
    TH2F *frame = new TH2F("frame", "m_{stop} vs. m_{lsp} - ATLAS work in progress", 100, 95, 350, 100, 50, 350. ); //JM
    // set common frame style
    CombinationGlob::SetFrameStyle2D( frame, 1.0 ); // the size (scale) is 1.0
    
    frame->SetXTitle( "m_{#tilde{t}_{1}} [GeV]" ); //JM
    frame->SetYTitle( "m_{#tilde{#chi}^{0}_{1}} [GeV]" ); //JM
    ...
    TLegend *leg = new TLegend(0.15,0.70,0.40,0.55); //JM 0.6,0.7,0.92,0.9
    ...
    if (true) { // expected and observed
		...
		for (int xbin = 1; xbin <= contour_expcls->GetNbinsX(); xbin++) {
			for (int ybin = 1; ybin <= contour_expcls->GetNbinsY(); ybin++) {
				// island 2
				if ( ( ax->GetBinCenter( xbin) > 190. && ax->GetBinCenter( xbin) < 210. &&
					   ay->GetBinCenter( ybin) > 170. && ay->GetBinCenter( ybin) < 190.) ||
					 ( ax->GetBinCenter( xbin) > 160. && ax->GetBinCenter( xbin) < 170. &&
					   ay->GetBinCenter( ybin) > 130. && ay->GetBinCenter( ybin) < 140.) ||
					 ( ax->GetBinCenter( xbin) > 150. && ax->GetBinCenter( xbin) < 160. &&
					   ay->GetBinCenter( ybin) > 110. && ay->GetBinCenter( ybin) < 140.) ||
					 ( ax->GetBinCenter( xbin) > 120. && ax->GetBinCenter( xbin) < 140. &&
					   ay->GetBinCenter( ybin) > 90. && ay->GetBinCenter( ybin) < 100.) ||
					 ( ax->GetBinCenter( xbin) > 105. && ax->GetBinCenter( xbin) < 115. &&
					   ay->GetBinCenter( ybin) > 80. && ay->GetBinCenter( ybin) < 90.) ) {
					cout << "Found spot here: " << xbin << " (" << ax->GetBinCenter( xbin)  << "), "
						 << ybin << " (" << ay->GetBinCenter( ybin) << "), "
						 << " value: " << contour_expcls->GetBinContent(xbin,ybin) <<   endl;
					cout << "   HACK : Setting above point by hand to 2.00 (!)" << endl;
					contour_expcls->SetBinContent(xbin, ybin, 2.00);
					contour_obs->SetBinContent(xbin, ybin, 2.00);
					contour_ep1s->SetBinContent(xbin, ybin, 2.00);
					contour_em1s->SetBinContent(xbin, ybin, 2.00);
					contour_esigxsp1s->SetBinContent(xbin, ybin, 2.00);
					contour_esigxsm1s->SetBinContent(xbin, ybin, 2.00);
				}
			}
        }
    }
    ...
    if (discexcl==1) {
        ...
        if (!extExpectation) { 
            if (showOneSigmaExpBand) {
                ...
                DummyLegendExpected(leg, "Expected limit (#pm1 #sigma_{exp})", c_myYellow, 1001, CombinationGlob::c_DarkBlueT3, 6, 2); //JM, shift it
    ...
    TString plottitle = "#tilde{t}_{1}#tilde{t}_{1} production #tilde{t}_{1} #rightarrow c#tilde{#chi}^{0}_{1}"; //JM
    ...
    Leg2->SetTextFont(70);
    if (prefix!=0) { 
        Leg2->DrawLatex(0.15,0.85,prefix); // 0.15,0.81 or 0.7, 0.85
        Leg2->AppendPad(); 
    }
    ...
    if(showSR){
        ...
        //Show_UL(fname0, c2, xmin, xmax, ymin, ymax, useShape, leg);

        //JM//std::cout << "--- printing best SRs" << std::endl;
...
void Show_SR(TString oredList,  TCanvas *can, float xlow, float xhigh, float ylow, float yhigh, bool useShape, TLegend *leg)
{
    ...
    for( Int_t i = 0; i < tree->GetEntries(); i++ ){
		...
		*/	
		...
		//TString mySR = GetSRCLs(fID, useShape);
		if (m0 < 350)
    ...
    if (oredList.Contains("GG")) {
        ...
        line2 = new TLine( 1150, 1565, 1260, 1565); 
        cout << "SG line1" << endl;
    } else if (oredList.Contains("SM_TT_directCC")) {
        line1 = new TLine( 135, 50, 350, 265); 
        line2 = new TLine( 95, 95, 350, 350); 
        cout << "SM_TT_directCC lines" << endl;
    }

    line1->SetLineWidth(0.9);
    line1->SetLineColor(CombinationGlob::c_LightGray);
    line1->SetLineStyle(2);
    line1->Draw("SAME") ;

    line2->SetLineWidth(0.9);
    line2->SetLineColor(CombinationGlob::c_LightGray);
