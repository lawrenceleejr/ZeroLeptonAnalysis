...
def makeNicePlots(result,workspace,SRname):
  ...
  for idx in range(fpf.getSize()):
    ...
    if parname.find("alpha_")>=0:
      infoForNPPlots.append((parname, par.getVal(),par.getError()))
    if parname.find("mu_")>=0 and  parname.find("stat")<0 and  parname.find("mu_SIG")<0:
      infoForMUPlots.append((parname.replace("mu_",""), par.getVal(),par.getError()))


  canvas_NP=TCanvas("TC_NP","TC_NP")
  canvas_NP.SetBottomMargin(0.2)
  h_NP=TH1F("NP","Nuisance parameters for "+SRname,len(infoForNPPlots),0,len(infoForNPPlots))
  h_NP.SetMarkerStyle(20)
  h_NP.GetXaxis().SetLabelSize(0.05);
  ...
  canvas_MU=TCanvas("TC_MU","TC_MU")
  h_MU=TH1F("MU","Strengh parameters for "+SRname,len(infoForMUPlots),0,len(infoForMUPlots))
  h_MU.SetMarkerStyle(20)
  h_MU.GetXaxis().SetLabelSize(0.08);
  ...
  numPars=0
  for idx in range(fpf.getSize()):
    if fpf[idx].GetName().find("gamma") != -1 and fpf[idx].GetName().find("VR") != -1:
      continue
    #print "Found param %s" % fpf[idx].GetName()
    numPars+=1

  #print "numPars = %d" % numPars
  oldMatrix = TH2D(result.correlationHist("h_corr_"+result.GetName()))

  corrMatrix = TH2D("new","",numPars,0,numPars,numPars,0,numPars)
  corrMatrix.SetTitle(SRName)
  ...
  for i in range(1, fpf.getSize()+1):
    ...
    for j in range(1, fpf.getSize()+1):
      yLabel = oldMatrix.GetYaxis().GetBinLabel(j)
  ...
  c_corr = TCanvas("TC_cor","TC_cor",600,400);
  c_corr.SetLeftMargin(0.3)
  ...
  elif(numPars<10):   labelSize = 0.04;
  elif(numPars<20):   labelSize = 0.025;
  elif(numPars<40):   labelSize = 0.02;
  else: labelSize = 0.015;

  corrMatrix.GetXaxis().SetLabelSize(labelSize);
  corrMatrix.GetYaxis().SetLabelSize(labelSize);
...
def latexfitresults_method2(filename,resultname='RooExpandedFitResult_afterFit', region='3jL', sample='', fitregions = 'WR,TR,S3,S4,SR3jT,SR4jT', dataname='obsData', doAsym=False, SRName=""):
  """
...
if __name__ == "__main__":
  ...
  def usage():
    ...
    print "-n <SRname>: name of the signal region"
  ...
  wsFileName=''
  try:
    opts, args = getopt.getopt(sys.argv[1:], "o:c:w:m:n:f:s:%by")
  except:
    usage()
  if len(opts)<2:
    usage()
