...
if configMgr.readFromTree:
    ...
    if zlFitterConfig.useFilteredNtuples:
        ...
        # Diboson
        dibosonFiles.append(os.path.join(INPUTDIR, "/BKG/Diboson.root"))
  
        # Top
        topFiles.append(os.path.join(INPUTDIR, "/BKG/Top.root"))
        if not zlFitterConfig.usePreComputedTopGeneratorSys:
            zFiles.append(os.path.join(INPUTDIR, "TopaMcAtNloHerwigpp.root"))
        if not zlFitterConfig.usePreComputedTopFragmentationSys:
            topFiles.append(os.path.join(INPUTDIR, "TopPowhegHerwigpp.root"))


        # W
        wFiles.append(os.path.join(INPUTDIR, "WMassiveCB.root"))
        if not zlFitterConfig.usePreComputedWGeneratorSys:
            wFiles.append(os.path.join(INPUTDIR, "WMadgraphPythia8.root"))

        # Z
        zFiles.append(os.path.join(INPUTDIR, "ZMassiveCB.root"))
        if not zlFitterConfig.usePreComputedZGeneratorSys:
            zFiles.append(os.path.join(INPUTDIR, "ZMadgraphPythia8.root"))
        
        # gamma
        gammaFiles.append(os.path.join(INPUTDIR, "GAMMAMassiveCB.root"))
    
    #data
    dataFiles.append(os.path.join(INPUTDIR_DATA, "DataMain_Nov01.root"))
