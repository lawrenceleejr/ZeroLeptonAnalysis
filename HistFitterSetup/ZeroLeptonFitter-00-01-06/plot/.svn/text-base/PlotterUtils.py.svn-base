...
#    print 'makecombined',centralHisto.GetNbinsX() iBin in range(0,centralHisto.GetNbinsX()):   
    for  iBin in range(1, centralHisto.GetNbinsX()):
        errUp2=0
        errDown2=0
...
#                errDown2 += math.pow((varSimm[iUDvar].GetBinContent(iBin)-centralHisto.GetBinContent(iBin))/centralHisto.GetBinContent(iBin),2);
        ...
        ErrorBand.SetPoint(iBin-1,centralHisto.GetBinCenter(iBin),1);
        ErrorBand.SetPointEXhigh(iBin-1,centralHisto.GetBinWidth(iBin)/2.); 
        ErrorBand.SetPointEXlow(iBin-1,centralHisto.GetBinWidth(iBin)/2.); 
        ErrorBand.SetPointEYhigh(iBin-1,math.sqrt(errUp2));
        #print iBin-1,errUp2,errDown2
        if errDown2 >= 1.:
            ErrorBand.SetPointEYlow(iBin-1,0.999);
        else:
            ErrorBand.SetPointEYlow(iBin-1,math.sqrt(errDown2));
#	print 
