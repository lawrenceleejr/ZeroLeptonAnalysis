...
void SUSY_gtt_all_witobs_merged( TString fname1 = "m0m12_nofloat_exp.root",
        ...
        int showtevatron = 0,
        TString plottitle = "0 leptons, 2-6 jets",
        bool showSR = true,
        bool useShape = false,
        ...
        )
{
    ...
    gtt.SetTextColor(14);
    gtt.SetTextAngle(90 - atan2(ymaxLine-yminLine, xmaxLine-xminLine) * 180/TMath::Pi() ); 
    gtt.SetTextFont(42);
    gtt.Draw("same");
    ...
    if (contour!=0 && discexcl==1)
    {
        ...
        // yes, these use weird names.
        TGraph* gcontour_om1s = ContourGraph( contour_obs_unc )->Clone();
        TGraph* gcontour_op1s = ContourGraph( contour_exp_all )->Clone();
    ...
    TLatex *Leg0 = new TLatex( xmin, ymax + dy*0.025, "#tilde{g}-#tilde{g} production, #tilde{g}#rightarrow #tilde{t}t #rightarrow ct#tilde{#chi}^{0}_{1}, #Deltam(#tilde{t},#tilde{#chi}^{0}_{1}) = 20 GeV" );
    ...
    if(showsig){
        ...
        axistitle.SetTextFont(42);
        axistitle.SetNDC();
        axistitle.DrawLatex(0.99,0.18,"Numbers give 95% CL excluded model cross sections [fb]");
    }	
    frame->Draw( "sameaxis" );
    ...
    TLatex clslimits = TLatex(1190,940,"All limits at 95% CL");
    ...
    progressLabel->DrawLatex(0.315, 0.85,"Internal");
    ...
    TLine obsPOneSigma = TLine(420,755,496,755); //used to be 633/655; width should be 51, height diff with low 22
    obsPOneSigma.SetLineStyle(3);
    obsPOneSigma.SetLineWidth(2);
    obsPOneSigma.SetLineColor(c_myRed);
    obsPOneSigma.Draw("same");

    TLine obsMOneSigma = TLine(420,723,496,723);
