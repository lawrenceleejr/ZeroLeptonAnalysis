...
void MUED_R_vs_RLambda_all_withBand_cls_blind( TString& fname0 = "mudat_list.root",// nominal
        TString fname1 = "",               // Up
        TString fname2 = "",               // Down  
        ...
        TString hname6 = "sigp1ref")
{
    ...
    cout << "--- Plotting1/R versus R*Lambda " << endl;
    ...
    TString title=  "1/R vs #Lambda.R - ATLAS for approval";
    TH2F *frame = new TH2F("frame", title, 40,650., 1200. ,40, 2.5, 60.);
    float xlow = 600;
    float xhigh = 1200;
    float ylow = 0;
    float yhigh = 60;
    ...
    frame->SetXTitle( "1/R [GeV]" );
    frame->SetYTitle( "#Lambda.R" );
    ...
    Int_t nsigma=2;
    TLegend *leg = new TLegend (0.15,0.83,0.35,0.92);
...
//fix not showing values under 1/R=700GeV
   graph = new TGraph(4);
   graph->SetName("Graph");
   graph->SetTitle("Graph");
   graph->SetFillColor(kWhite);
   graph->SetLineColor(kWhite); 
   graph->SetPoint(0,600,3);
   graph->SetPoint(1,600,45);
   graph->SetPoint(2,701,45);
   graph->SetPoint(3,701,3);
   graph->Draw("same cf");
...
if(show7TeVlimits){
    c->cd();
    graph_7TeV= SimplifiedModels_7TeV("MUED") ; 
...
if (discexcl==1) {
    ...
    if (!extExpectation) { // expectation from toys
        ...
        if (!blind){
            ...
            if (contour_obscls!=0) DrawContourLine95( leg, contour_obscls, "Observed limit ", c_myRed, 1, 4);
        }
...
//fix not showing values under 1/R=700GeV
   ...
   graph->SetFillColor(kWhite);
   graph->SetLineColor(kWhite); 
   graph->SetPoint(0,600,2);
   graph->SetPoint(1,600,70);
   graph->SetPoint(2,900,70);
   graph->SetPoint(3,900,40.4);
   graph->SetPoint(4,704,40.4);
   graph->SetPoint(5,704,2);
   graph->SetPoint(6,600,2);
   graph->Draw("same f");
...
// draw a line   
   if(!showSR){   
     ...
     TLine* line1  = new TLine( 650, 40, 1200, 40);
     line1->SetLineWidth(2);
     line1->SetLineColor(kBlack);
     line1->SetLineStyle(1);
     line1->Draw("SAME") ;
   
     TLine* line11  = new TLine( 650, 40.6, 1200, 40.6);
     line11->SetLineWidth(10);
     line11->SetLineColor(kWhite);
...
TString LegendTitle="minimal Universal Extra Dimensions model" ; 
...
Leg2->SetTextFont(42);
if (prefix!=0) {
    Leg2->DrawLatex(0.72,0.88,"Internal");
    Leg2->DrawLatex(0.16,0.81,prefix); // 0.15,0.81
...
TGraph* ContourGraph( TH2F* hist)
{
	...
	for (int i = 0 ; i<N ; ++i){
	   ...
	   if (x<700.) grTmp->SetPoint(i,600.,y);
	}
...
TObjArray ContourGraphs( TH2F* hist)
{
   ...
   for (int k = 0 ; k<number_of_lists ; k++){
      ...
      for (int i = 0 ; i<N ; ++i){
	 ((TGraph *)gr_vec.At(k))->GetPoint(i,x,y);
	 cout << "i= " << i << "  x= " << x << " y= " << y << endl;
	 if (x<700.) ((TGraph *)gr_vec.At(k))->SetPoint(i,600.,y);
      }
   }
...
void Show_SR(TString oredList,  TCanvas *can, float xlow, float xhigh, float ylow, float yhigh, bool useShape)
{
    ...
    TBranch *b_m3;
    TBranch *b_fID;
    tree->SetBranchAddress("LambdaR", &m3, &b_m3);
    tree->SetBranchAddress("oneOverR", &m1, &b_m1);
        ...
        float ylow, float yhigh, bool acceff=false)
{
    ...
    TString ULstring;
    char buffer[20];
    float m1_old=0, m3_old=0;
    for( Int_t i = 0; i < tree->GetEntries(); i++ ){
        tree->GetEntry( i );

        if (m1!=m1_old) m1_old= m1;
        if (m3!=m3_old) m3_old= m3;
    ...
    TLatex *Leg0 = new TLatex( xhigh+70, ylow + 5, LegendTitle);
