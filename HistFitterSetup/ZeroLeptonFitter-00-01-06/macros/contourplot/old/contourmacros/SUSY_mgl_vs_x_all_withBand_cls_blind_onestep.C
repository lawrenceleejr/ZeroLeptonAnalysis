...
void SUSY_mgl_vs_x_all_withBand_cls_blind_onestep( TString& fname0 = "mudat_list.root",// nominal
        TString fname1 = "",               // Up
        TString fname2 = "",               // Down  
        ...
        TString hname6 = "sigp1ref")
{
    ...
    cout << "--- Plotting mgluino/squark versus x " << endl;
    ...
    TH2F* contour_au1s(0);
    if (histe_u1s!=0) {  contour_au1s   = FixAndSetBorders( *histe_u1s, "contour_au1s", "contour_ad1s", 0 ); }
    TH2F* contour_ad1s(0);
    ...
    if(fname0.Contains("GG_onestep")) ContourFileName="GG_onestep_LSP60_Countours.dat";
    else ContourFileName="SS_onestep_LSP60_Countours.dat";
	...
	TObjArray gr_contour_ep1s_vec = (TObjArray)ContourGraphs( contour_ep1s ).Clone(); 
        TObjArray gr_contour_em1s_vec = (TObjArray)ContourGraphs( contour_em1s ).Clone(); 
    }
    ...
    TCanvas* c = new TCanvas( "c", "A scan of m_{squark/gluino} versus x", 0, 0, 
            CombinationGlob::StandardCanvas[0], CombinationGlob::StandardCanvas[0]-10 );  
    //c->SetGrayscale();

    // create and draw the frame
    TString title;
    if(fname0.Contains("GG_onestep")) title=  "m_{#tilde{g}} vs x - ATLAS for approval";
    else if(fname0.Contains("SS_onestep")) title=  "m_{#tilde{q}} vs x - ATLAS for approval";
    float xlow=170., xhigh=1600., ylow=0., yhigh=1.5;
    if(fname0.Contains("SM_SS_onestep")) xhigh= 1050.;
    ...
    if(fname0.Contains("GG_onestep"))  frame->SetYTitle("x=#Deltam(#tilde{#chi}^{#pm}_{1},#tilde{#chi}_{1}^{0})/#Deltam(#tilde{g},#tilde{#chi}_{1}^{0})" );
    else if(fname0.Contains("SS_onestep"))   frame->SetYTitle("x=#Deltam(#tilde{#chi}^{#pm}_{1},#tilde{#chi}_{1}^{0}))/#Deltam(#tilde{q},#tilde{#chi}_{1}^{0}))" );
    ...
    //TColor *col_myYellow = gROOT->GetColor(c_myYellow); 
    //col_myYellow->SetAlpha(0.01);
    Int_t c_myRed      = CombinationGlob::c_DarkRed;
    Int_t c_myExp      = CombinationGlob::c_DarkBlueT3;
    c->cd();

    // draw 7TeV observed limits
    TGraph *graph_7TeV(0);
    if(show7TeVlimits){
        c->cd();
        if (fname0.Contains("SS")) graph_7TeV= SimplifiedModels_7TeV("SM_SS_onestep_LSP60") ;
        else if (fname0.Contains("GG"))graph_7TeV= SimplifiedModels_7TeV("SM_GG_onestep_LSP60") ; 
        leg->AddEntry(graph_7TeV,"Observed limit (4.7 fb^{-1}, 7 TeV)","f");
    }

    //TGraph* grshadeExp = DrawExpectedBand( gr_contour_em1s, gr_contour_ep1s, CombinationGlob::c_DarkYellow , 1001   , 0)->Clone();
    TGraph* grshadeExp = DrawExpectedBand_new( gr_contour_em1s, gr_contour_ep1s, c_myYellow , 1001,
            xlow,xhigh,ylow,yhigh,//200, 1000, 0, 0,
            0, 0)->Clone();

    //fix second contour of expected 1sigma exclusion line
    if(gr_contour_ep1s_vec.GetEntries()>1) {
      TGraph *gr= (TGraph *)gr_contour_ep1s_vec.At(1);
      //gr->SetFillColor(kWhite);
      gr->SetFillColor(1001);
      gr->Draw("same f");    
    }
    ...
    if(show7TeVlimits){
	  ...
	  SimplifiedModels_7TeV("SM_SS_onestep_LSP60_small") ;
	  SimplifiedModels_7TeV("SM_SS_onestep_LSP60",false);
	}  
        else if (fname0.Contains("GG")) SimplifiedModels_7TeV("SM_GG_onestep_LSP60",false) ; 
    }
    ...
    if (discexcl==1) {
        ...
        if (!extExpectation) { // expectation from toys
            ...
            TLine* line1;
	    if(fname0.Contains("SS")) line1= new TLine( 210, 1.375, 246, 1.375);
            else line1= new TLine( 237, 1.375, 294, 1.375);
            line1->SetLineWidth(2);
            line1->SetLineColor(CombinationGlob::c_DarkRed);
            line1->SetLineStyle(3);
            line1->Draw("SAME") ;
            TLine* line2;
	    if(fname0.Contains("SS")) line2= new TLine( 210, 1.315, 246, 1.315);
            else line2= new TLine( 237, 1.315, 294, 1.315);
    ...
    if (prefix!=0) {
        ...
        Leg2->DrawLatex(0.16,0.75,"m_{#tilde{#chi}_{1}^{0}}=60 [GeV]");
        Leg2->AppendPad(); 
    }
    ...
    if(yhigh > 2) {
        ...
        TString badLSPstr = "";
        if (grid.Contains("SS")) badLSPstr = "#tilde{q} LSP";
        else if (grid.Contains("GG")) badLSPstr = "#tilde{g} LSP";
        ...
        equalmass->SetPoint(1,topright,topright);
        equalmass->SetPoint(2,xlow,yhigh);
        equalmass->SetPoint(3,bottomleft,bottomleft);
        equalmass->Draw("f");
        equalmass->Draw("l");

        badLSP.DrawLatex(bottomleft+0.2*(xhigh-bottomleft)-20,bottomleft+0.2*(yhigh-bottomleft)+20,badLSPstr);
    }
    else { // draw line at position x=1
	TLatex badLSP;
	badLSP.SetTextFont( 42 );
	badLSP.SetTextSize( 0.0265 );
    ...
    if (fname0.Contains("GG")){  m1= 1200; m2= 1150; m3= 60;}
    else if (fname0.Contains("SS")){ m1=700; m2=675;  m3==60;}
    
    float x= (m2 -m3)/(m1-m3);
    c->cd();
    marker.DrawMarker(m1,x); 
    ...
    //draw best SRs
    cout << "showsr= " << showSR << endl;
    if(showSR){
        cout << "in SHow_SR()"<<endl;
...
TGraph* ContourGraph( TH2F* hist)
{
    ...
    cout << "conturs size= " << ncontours << endl;
    TList *list = (TList*)contours->At(0);
    Int_t number_of_lists = list->GetSize();
    cout << "list size= " << number_of_lists << endl;
...
TObjArray ContourGraphs( TH2F* hist)
{
   TH2F* h = (TH2F*)hist->Clone();
   h->SetContour( 1 );
   double pval = CombinationGlob::cl_percent[1];
   ...
   Double_t x=0, y=0;
   TObjArray gr_vec;
   if(gr_vec.GetEntries()>0) gr_vec.Clear();
   char buffer[100];
   for (int k = 0 ; k<number_of_lists ; k++){
      gr_vec.Add((TGraph*)list->At(k));
      sprintf(buffer,"%s_%d",hist->GetName(),k);
      ((TGraph *)gr_vec.At(k))->SetName(buffer);
      Int_t N = ((TGraph *)gr_vec.At(k))->GetN();
   }
   if(gr_vec.GetEntries()>1) cout << ((TGraph *)gr_vec.At(1))->GetN() << endl;
   
   return SortArray(gr_vec);
}

TObjArray SortArray(TObjArray array){
  Int_t NumPoints[100];
  Int_t sortedIndexes[100];
  Int_t N=0;
  for(Int_t i=0; i<array.GetEntries(); ++i){
    NumPoints[i]= ((TGraph *)array.At(i))->GetN();
    N++;
  }
  
  TMath::Sort(N,NumPoints,sortedIndexes,true);
  TObjArray sortedArray;
  for(Int_t i=0; i<array.GetEntries(); ++i){
    sortedArray.Add(array.At(sortedIndexes[i]));
  }  
  return sortedArray;
}
...
void DumpContours( TH2F* hist, ofstream &ContourFile, TFile *RootFile,TString CLsType)
{
    ...
    //TGraph* grTmp = new TGraph();
    for (int k = 0 ; k<number_of_lists ; k++){        
        TGraph* grTmp = (TGraph*)list->At(k);
	Int_t N = grTmp->GetN();
	ContourFile << "countour   "  << k+1 << "   Npoints= " << N << "\n";
	...
	for(int j=0; j<N; j++) {
	  ...
	  //sprintf(buffer," %.1f; %.3f;\n",xx0,yy0);
	  //ContourFile<< buffer; 
	  xarray[j]= xx0;
	  yarray[j]= yy0; 
        }
	
	int begin_yminus=-999, end_yminus=-999, minimum=0;
	int point_counter=0;
	for(int j=0; j<N; j++){
	  if (xarray[j]< xarray[minimum]) minimum=j;
	...
	if( begin_yminus==-999 && end_yminus==-999) begin_yminus=N;
...
void Show_SR(TString oredList,  TCanvas *can, float xlow, float xhigh, float ylow, float yhigh, bool useShape)
{
    ...
    TString SRFileName="";
    if(oredList.Contains("GG_onestep")) SRFileName="GG_onestep_LSP60_SR.dat";
    else SRFileName="SS_onestep_LSP60_SR.dat";
    char buffer[100];
	...
	if(oredList.Contains("SS") && m1>1050) continue;
	...
	TString mySR = GetSRName(fID, useShape);
        ...
        //lat.DrawLatex(m1,x,mySR.Data());
        if(!(m1>500 && (m1-m2==25 || m2== m3+25 || m1-m2==50))){
	  lat.DrawLatex(m1,x,mySR.Data());      
...
void Show_UL(TString oredList, TCanvas *can,bool observed=false,float xlow,float
        xhigh,float ylow,float yhigh, bool acceff=false)
{
    can->cd();
    ...
    TString ULFileName="";
    if(oredList.Contains("GG_onestep")) ULFileName="GG_onestep_LSP60_UL_SR.dat";
    else ULFileName="SS_onestep_LSP60_UL_SR.dat";
    ...
    if(oredList.Contains("GG_onestep")) {
      myfile << "*location: Auxiliary Figures 23a, 24a\n";
      ...
      myfile << "*location: Auxiliary Figures 23c, 24c\n";
    ...
    myfile << "*obskey: M\n";
    myfile << "*obskey: X\n";
    if(oredList.Contains("GG_onestep")) myfile << "*qual: RE : P P --> GLUINO < QUARK QUARKBAR W NEUTRALINO1 > GLUINO < QUARK QUARKBAR W NEUTRALINO1 > \n";
    else myfile << "*qual: RE : P P --> SQUARK < QUARK W NEUTRALINO1 > SQUARK < QUARK W NEUTRALINO1 >  \n";
    myfile << "*qual: SQRT(S) IN GEV : 8000.0\n";
    myfile << "*qual: . : NEUTRALINO MASS = 60 GEV \n";
    myfile << "*yheader: Cross-section limit IN FB : noZ SRs \n";
    if(oredList.Contains("GG_onestep")) myfile << "*xheader: GLUINO MASS IN GEV : X=DM(CHARGINO1 - NEUTRALINO1)/DM(GLUINO - NEUTRALINO1)\n";
    else myfile << "*xheader: SQUARK MASS IN GEV : X= DM(CHARGINO1 - NEUTRALINO1)/DM(SQUARK - NEUTRALINO1)\n";
    myfile << "*data: x : x : y : y \n";

    if (oredList.Contains("GG_onestep")) tree->BuildIndex("mgluino","mchargino");
    else tree->BuildIndex("msquark","mchargino");
	...
	if(UL*xsec>=1000) sprintf(buffer," %d; %.3f; %.0f; %s;\n",m1,x,UL*xsec,mySR.Data());
	else if(UL*xsec>=100) sprintf(buffer," %d; %.3f; %.1f; %s;\n",m1,x,UL*xsec,mySR.Data());
	else if(UL*xsec>=10) sprintf(buffer," %d; %.3f; %.2f; %s;\n",m1,x,UL*xsec,mySR.Data());
	else if(UL*xsec>=1) sprintf(buffer," %d; %.3f; %.3f; %s;\n",m1,x,UL*xsec,mySR.Data());
	else if(UL*xsec>=0.1) sprintf(buffer," %d; %.3f; %.4f; %s;\n",m1,x,UL*xsec,mySR.Data());
	else sprintf(buffer," %d; %.3f; %.5f; %s;\n",m1,x,UL*xsec,mySR.Data());
	myfile << buffer;
	...
	if(oredList.Contains("SS") && m1>1050) continue;
       ...
       if(acceff) sprintf(buffer,"%.1f",UL*xsec);
        ...
        ULstring=buffer;
        cout << "  At x = " << m1<< ", y = " << x<< ", UL = " <<  UL
            << ", ULstring= " << ULstring<< endl;

        //lat.DrawLatex(m1,x,ULstring.Data());
        if(!(m1>500 && (m1-m2==25 || m2== m3+25 || m1-m2==50)) && UL*xsec<1000000.) {
	  lat.DrawLatex(m1,x,ULstring.Data());      
	}  
    ...
    TLatex *Leg0;
    if(oredList.Contains("SS_onestep")) Leg0 = new TLatex( xhigh+40, ylow+0.03*(yhigh-ylow), LegendTitle);
    else  Leg0 = new TLatex( xhigh+65, ylow+0.03*(yhigh-ylow), LegendTitle);
