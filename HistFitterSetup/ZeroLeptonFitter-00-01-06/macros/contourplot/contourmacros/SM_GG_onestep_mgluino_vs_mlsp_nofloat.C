...
const char* SM_GG_onestep_mgluino_vs_mlsp_nofloat(const char* textfile = 0, TH2D* inputHist = 0, const char* rootfile = "mgluinomlsp_nofloat.root", TString id1="mgluino",TString id2="mlsp", int   nbinsX=100,int nbinsY=100, float minX=0,float maxX=1600, float minY=0, float maxY=1600)
{
  ...
  if (inputHist!=NULL){
   ...
   if (inputHist!=NULL){
      ...
      TH2D* p0exp = DrawUtil::triwsmooth( tree, "StatTools::GetSigma(p0exp):"+correlation, "p0exp" , "One-sided significance p0 (exp)", "(p0exp>0 && p0exp<=1)", clonep0exp );}
   else{
      TH2D* p0exp = DrawUtil::triwsmooth( tree, "StatTools::GetSigma(p0exp):"+correlation, "p0exp" , "One-sided significance p0 (exp)", "(p0exp>0 && p0exp<=1)", inputHist );}
   ...
   if (inputHist!=NULL){
      ...
      TH2D* p0expraw = DrawUtil::triwsmooth( tree, "p0exp:"+correlation, "p0expraw" , "One-sided significance p0 (exp)", "(p0exp>0 && p0exp<=1)", clonep0expraw );}
   else{
      TH2D* p0expraw = DrawUtil::triwsmooth( tree, "p0exp:"+correlation, "p0expraw" , "One-sided significance p0 (exp)", "(p0exp>0 && p0exp<=1)", inputHist );}
